...
#
# 为什么必须有这个脚本：vhost 里那条 `ssl_protocols TLSv1.3`（刻意不 include
# certbot 的 options-ssl-nginx.conf，因为 nginx 对该指令是**并集**语义）如果用眼睛看、
# 用 `nginx -t` 验、用 grep 找，**三道看似是证明的东西全是假的** —— 实测过 include +
# 覆盖的写法仍然协商成功 TLS 1.2，而 nginx -t 只报一句 duplicate value 的警告。
# 唯一能证伪的是一次真握手。不把这套流程固化下来，它下次还是会被「看起来对」骗过去。
#
...
# 否则下面「TLS 1.2 被拒绝」可能只是因为判据恒返回空（空转恒绿）。
...
if [ -n "${P12}" ]; then
  fail "TLS 1.2 握手成功（协商 Protocol=${P12}）—— ssl_protocols 是并集语义，\
include 了 options-ssl-nginx.conf 就一定会这样。见 vhost 里那段注释。"
fi
pass "TLS 1.2 被拒绝（alert protocol version，无协商结果）"

P11="$(tls_proto -tls1_1)"
if [ -n "${P11}" ]; then
  fail "TLS 1.1 握手成功（协商 Protocol=${P11}）—— 更严重"
fi
pass "TLS 1.1 被拒绝"
