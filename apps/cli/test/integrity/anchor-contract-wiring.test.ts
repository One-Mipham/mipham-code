...
describe('anchor 契约接线', () => {
  ...
  it('空转守卫：抽取脱钩（清空 / 同义反复）、多认条目（谓词析取项）、或委派被架空时不得静默全绿', async () => {
    ...
    // ⚠️ **证据要拆成两格说（终审实测订正）**：这两个分句**在任何单一磁盘状态上都不共存** ——
    // **不带**真降级时本文件 4 绿，但那时 `baseline` **并没有**被污染；**带**真降级 `:405` 时
    // `baseline` 确被污染，而本文件是 **1 failed（红在 clos3）**、**不是** 4 绿。
    // ⇒ 「没有蕴含关系」这句话**是真的**，但它**不由**「本文件仍 4 绿」支持 —— 别把后半句当证据读
    // （照字面读会得出反结论，进而可能去删一处**正确**的登记）。
