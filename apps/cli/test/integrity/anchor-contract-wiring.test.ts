...
describe('anchor 契约接线', () => {
  ...
  it('空转守卫：抽取脱钩（清空 / 同义反复）、多认条目（谓词析取项）、或委派被架空时不得静默全绿', async () => {
    ...
    // **不**守「clos2 的 `baseline` 来自真产出」—— 这两件事之间没有蕴含关系。
    // ⚠️ **证据要按载体分格说（终审 + 复评实测订正）—— 别把它读成全称**：
    //  · **不带**真降级 ⇒ 本文件 4 绿，且 `baseline` **并没有**被污染（探针：与真产出逐 id 相同，17 == 17）；
    //  · **无状态载体**（`heal_nonenum` 这类每次调用都撒谎的）**+ 真降级 `eval-harness.ts:405`**
    //    ⇒ `baseline` **确被污染**（探针：17 个含 `merge-convergence-gate` vs 真产出 16 个），
    //    而本文件是 **1 failed（红在 clos3）**、**不是** 4 绿；
    //  · **无载体** + 同一处真降级 ⇒ **3 failed**（`it#1` / `clos3` / `it#4`）
    //    ⇒ **别把「带真降级」单独读成 1 failed**；
    //  · **有状态载体** + 同一处真降级 ⇒ 本文件 **4 绿 而 `baseline` 已被污染** —— 见文件头第三条
    //    残留轴，`:77-79` 自记「真删一处 `anchor: true` 照样 4 绿」。**这一格正是「4 绿」与「已被
    //    污染」共存的实例** ⇒ 本条**不是**「两者永不同盘」的全称；照全称读会推出「那一轴复现不出」
    //    ⇒ 进而删掉一条**正确**的登记（本批招牌缺陷类最坏的形态）。
    // ⇒ 「没有蕴含关系」仍是真的，只是它的证据必须**按载体分格**读，不能合成一句。
