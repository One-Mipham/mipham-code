...
3. **`CEILING` 是 `23145750`，不是校准文件里的 `10,000,000`**。两个数都写在这里：
   ...
   **照抄旧值**写回去的。于是一个被盖成 50505050 的账本**永久**是 50505050，`init` 每跑一次都
   如实打印这个数 —— 看着像「命令没生效」，实为「在这条路径上生效不了」。**补救只有换账本路径**：
   `MIPHAM_BENCH_LEDGER` 指向一个**新文件**（文件不存在时 `budget.py:35-36` 取构造值，也就是
   环境变量送进来的那个）。旧账本里的**已花记录不会跟过去** —— 换之前先想清楚那笔支出要不要留档。
