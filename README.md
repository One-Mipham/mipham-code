...
输入行支持 vim 双模式，按 `Esc` 切换，看提示符区分：

| 提示符 | 模式           | 行为                                                         |
| ------ | -------------- | ------------------------------------------------------------ |
| `>`    | insert（插入） | 正常打字；输 `/` 打开斜杠命令选择器                          |
| `:`    | normal（普通） | vim 快捷键（`h/j/k/l` 移动、`dd` 删行、`yy` 复制、`u` 撤销） |

> ⚠️ **踩坑提醒**：在 `:`（normal）模式下按 `/` 会进入 **搜索模式**，不是斜杠命令——此时 `/` 只是搜索框占位符，删不掉、输入也不进正文，容易误以为「锁死」。解法：搜索模式下 **backspace 删到空再按一下** 即退出，或直接按 `Esc` 切回 `>`（insert）模式再输 `/`。
