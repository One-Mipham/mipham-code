...
对标结论：**多数高价值项 Mipham Code 已覆盖或形式不同**（symlink TOCTOU、MCP 握手超时、worktree 隔离、prompt-cache 引擎层追踪均已实现）。真正值得动手的收敛为 **3 项**（见 §五）。
...
| --- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- | ---------------------------- |
| 1   | Bash 算术赋值绕过权限检查    | `security/gate.ts:30-38` 的 `DANGEROUS_BASH_PATTERNS` 只覆盖 4 类（`;rm/cat/sh/bash`、`curl\|sh`、`\|sh`、`>/dev/`），**未覆盖**整数 shell 变量算术赋值 | 高   | **不适用（无自动放行路径）** |
...
> 初版报告中标「待验证」的 5 项，本轮全部读完对应实现；#1 为本次复核新增的误判修正。每项附证据。

### 🔴 #1 Bash 算术赋值绕过 → **不适用（误判修正）**

**初版误判**：只读黑名单 `security/gate.ts:30-38`「未覆盖算术赋值」就下「确认缺失」结论，未读 Bash 权限决策流程——违反 read-first 原则。

**重读证据**（`core/permission.ts` + `tools/exec/bash.ts`）：

1. Bash 工具默认 `permission: 'ask'`（`bash.ts:313`），无「按内容自动放行」的黑名单分类器。
2. 权限解析链（`permission.ts:275-349`）唯一「按内容自动放行 Bash」的路径是 `acceptEdits` 模式的 `isVerificationCommand`。
3. `isVerificationCommand`（`permission.ts:17-61`）是**白名单**：① L22 严格排除 shell 元字符 `/[;&|><`$]/`；② 只匹配显式列出的验证命令（`pnpm test`/`tsc`/`git status`…）。
4. `OPTIND=1/0`、`RANDOM=2+2` 不含白名单命令名 → 落到 `ask`，**无自动放行路径**。

Claude Code 的漏洞前提是「auto mode 黑名单分类器按内容自动放行看似无害的命令」；Mipham 是「白名单 auto-approve」，**前提不成立**。唯一全量放行是 `bypassPermissions`（用户显式选择，非漏洞面）。
...
| ❌ **确认缺失，建议对标**    | model-switch hook（§二 #5）                                                                                                                                                                      |
| 🟡 **形式不同，需加固/补充** | 畸形 tool_use 显式校验（#9）、`/mcp connect` 信息披露（#3）                                                                                                                                      |
| ❓ **已核查，不适用**        | Bash 算术赋值绕过（#1）、Workflow scriptPath（#2）、effort+thinking（#7）、worktree 编辑（#10）                                                                                                  |
...
| 序  | 项                             | 性质     | 成本 | 说明                                                                                            |
| --- | ------------------------------ | -------- | ---- | ----------------------------------------------------------------------------------------------- |
| 1   | **#6 `/cost` 补缓存行**        | 展示     | 极低 | `context-token.ts` 的 `cachedCount/cachedTokens` 已在手，`/cost` 多打一行 hit ratio / warm-cold |
| 2   | **#9 畸形 tool_use 显式校验**  | 健壮性   | 低   | `engine.ts`/`openai-compat.ts` 对缺 `id`/`name` 的 tool_use 丢弃 + 触发干净重试，而非静默空 id  |
| 3   | **#3 `/mcp connect` 信息披露** | 安全加固 | 低   | connect HTTP server 前展示目标 URL + header 键名 + 来源分级（用户级 vs 项目级）                 |
...
**误判**：初版把 Claude Code 的「Bash 算术赋值绕过」漏洞机械套到 Mipham Code，只读黑名单 `security/gate.ts:30-38`「未覆盖算术赋值」就下「确认缺失」结论，未验证漏洞成立的前提（是否存在「按内容自动放行」的路径）。

**根因**：违反 read-first 原则——读了黑名单（阻断侧），没读权限决策流程（放行侧）。

**教训**：对标安全漏洞时，**黑名单缺某个模式 ≠ 有漏洞**。必须追「这条命令在自身权限模型里到底走哪条路径」——是自动放行（漏洞面），还是落到 ask/白名单（无漏洞面）。Mipham 是白名单 auto-approve（`isVerificationCommand`）+ 默认 `ask`，Claude Code 是黑名单分类器，前提不同，不能机械套用。
