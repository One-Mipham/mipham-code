...
> 头脑风暴中逐层做的选择。§3.1 是最终结论，本节省略了「被否决的选项」与「为何」，供后续「跑通后深入」时回访——决策路径一旦不记，下次会重新瞎琢磨或悄悄改决定而不自知。

| #   | 岔路口       | 选项                                                   | 选了 | 为何（否决项理由）                                                          | 推迟的（先跑通）                     | 回访触发                       |
| --- | ------------ | ------------------------------------------------------ | ---- | --------------------------------------------------------------------------- | ------------------------------------ | ------------------------------ |
| 1   | 代码来源     | A LLM 生成 / B 冻结片段 / C 分离式                     | A    | B 退化成静态检查、测不到 AI 行为；C 是架构不是代码来源（被 M1/M2 切分吸收） | —                                    | —                              |
| 2   | 噪声处理     | A 温度0单次 / B 多次采样 / C 固定小模型 / D 先最小可行 | A    | B 是 N× 成本；C 测的不是真实模型                                            | 统计严谨（最小效应阈值、多次采样）   | M2 做「改进 delta」时          |
| 3   | 任务集       | A 5-10 人工 / B 复用测试库 / C 先定 schema             | A    | 自包含、快、易维护                                                          | 任务池扩容 + 任务与被测 skill 相关性 | test-driven 跑通后             |
| 4   | 安全隔离     | A 子进程 / B 容器(Docker)                              | A    | 任务纯函数自包含、风险低；容器重                                            | 容器级隔离                           | 任务引入 I/O/网络/不可信依赖时 |
| 5   | bug-fix 类别 | A 首批做 / B 暂缓                                      | B    | bug-fix 需「bug 代码+红测试」冻结，构造成本高                               | bug-fix 类别                         | test-driven 跑通后按需补       |
| 6   | M1 vs M2     | A 先 M1 / B 一步到位 M2                                | A    | M2 依赖 skill 注入未细化，先出基准分数验证可行性                            | M2 before/after 接线                 | M1 跑通后                      |
| 7   | skill 注入   | 未定                                                   | —    | —                                                                           | skill 旧版/新版如何注入 LLM          | M2 启动时单独设计              |

**「先跑通」完整清单（5 项）**：噪声统计严谨、容器级隔离、bug-fix 类别、任务相关性冻结、M2 skill 注入。每项的「为何现在不做」见上表，与 §八风险逐条对应。
