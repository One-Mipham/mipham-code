...
**判据不是「容器终态」；但它也不是「verifier 一定跑在独立容器里」** —— 后一句是本计划**三处**未跟上的旧主张（本段、判据表那一行、Step 5 的提交信息模板 —— 三处逐一订正，见文末「真缺陷」条），**2026-09-18 按本任务自己的实测订正**。官方 harness 的 verifier 有**两种模式**：**只有 `SEPARATE`** 才跑在**独立的 verifier 容器**里、只读 `task.toml` 声明的 `artifacts`；**默认是 `SHARED`** —— verifier 在 agent **自己的**容器里跑（`harbor/models/task/verifier_mode.py:10-26`、`trial/trial.py:667` 分派）。**本任务实测：这十题全部是 `SHARED`**（`benchmarks/results/phase2-integration-gate.json` 的 `resolvedVerifierMode: "SHARED"`、`artifactsKeyPresent: false`，`tenTaskScan` 逐题扫过十题、**无一有 `artifacts` 键、也无 `verifier.environment`**）。两种模式下 verifier 都**不读我们的 chunk** —— 挡的是**归因**，不是**打分**。

**这对适配器的推论随模式而变，所以本任务必须实测「是哪一种」：** 在 `SEPARATE` 下，agent 在 `/testbed` 里改的东西要能被 verifier 看到，就必须被 `task.toml` 的 `artifacts` 声明覆盖 —— 声明不含改动路径时，**agent 干得再好也传不过去**，而症状（verifier 判失败）与「模型没做出来」在结果文件里长得一模一样。**在这十题上该机制整体不适用**（无 `artifacts` 键），但这**不是**「不用查」：正因为默认模式与直觉相反，本任务除下表外仍须读该题的 `task.toml`，把 `artifacts` **逐字抄进** `phase2-integration-gate.json` —— **如实记「没有」也是读数**（本次记的就是 `verdict: "N/A · 机制不适用"`）。**订正留痕**：同一主张在子仓 `CLAUDE.md` 2.48.0 行已由 `c8dc522` 订正为「`SEPARATE` 才独立、默认 `SHARED`」，本计划这几处是**未跟上的拷贝**，本次按对象补齐。
...
| **`task.toml` 的 `artifacts` 是否覆盖了 agent 改动的路径** | 先读出 `verifier_mode`，再定 | 见本节开头（**2026-09-18 订正**）：**只有 `SEPARATE`** 下 verifier 才在独立容器里只读声明的 artifacts；**默认是 `SHARED`**。**实测本题与十题全体都是 `SHARED`**、无 `artifacts` 键 ⇒ 该机制不适用，门里记 `verdict: "N/A · 机制不适用"`。**若**某题是 `SEPARATE` 而声明为空/不覆盖，则 agent 的活传不过去，而症状（verifier 判失败）与「模型没做出来」**在结果文件里长得一模一样** —— 那时先解决这一条再往下跑 10 题。无论如何都要读该题 `task.toml` 并把 `artifacts` 逐字抄进 `phase2-integration-gate.json`：**如实记「没有」也是读数** |
...
两个基准的任务形状相同，判分都由 harbor 的 verifier 做出 —— 该 verifier 在
SEPARATE 模式下跑独立容器、只读 task.toml 声明的 artifacts，而默认模式是 SHARED
（verifier 在 agent 自己的容器里跑；本题与十题全体实测均为 SHARED，无 artifacts 键）。
...
4. **（真缺陷，非笔误；且它的「订正」本身还有第二层，到 Task 16 实跑才暴露）** Task 16 初稿写「verifier 在容器终态上重打 `test_patch`」—— 这是**已在另两处订正掉的假主张**（子仓 `CLAUDE.md`、`docs/claude-md-history.md`），并据此给 Task 16 补了 `declaredArtifacts` 这一项。**但那次订正只对了一半**：它把「官方 verifier 跑在**独立容器**、只读 `task.toml` 声明的 `artifacts`」当成了**无条件**的机制，而实情是**两种模式** —— 只有 `SEPARATE` 才如此，**默认是 `SHARED`**（`verifier_mode.py:10-26`、`trial/trial.py:667` 分派）。**Task 16 一跑就把这第二层打出来了**：本题与十题全体都是 `SHARED`、无一有 `artifacts` 键，门里据实记 `verdict: "N/A · 机制不适用"`（见该任务提交 `aee7dd6` 正文，它在提交时即按实测订正了 brief 的这句断言）。本计划**上文那三处**（本节开头、判据表那一行、Step 5 的提交信息模板）与子仓 `CLAUDE.md` 2.48.0 行（`c8dc522`）现已一致。**「三处」与「四处」数的是不同的对象**：三 = 需要改的拷贝，就是上面列的这三处；四 = 再把**本条自己在叙述这次订正时的重述**算进去 —— 它也曾把同一机制当事实再断言一遍，修复波里一并改掉。**留下的教训不是「要订正」，而是「订正时别把『我在一个对象上读到的』写成『全称』」** —— 上一轮把 `SEPARATE` 从「一种模式」升成了「机制本身」，于是「agent 改的东西能不能到 verifier」这条实测断言，在十道没有该机制题上被要求去验证一个不存在的东西。**救它的是设计而不是运气**：`declaredArtifacts` 是**逐字抄 `task.toml`**，抄到的就是「没有」—— **如实记「没有」也是读数**。
...
- 2026-09-18 —— **上限口径定档（留痕，不改本计划任何已完成步骤）**。本计划 Task 15 的那句
  规则句「上限 = 10 ×（Phase 1 已完成题目的**每题材** token 中位数与均值中的较大者，向上取整到
  10 万）」，**已被裁决为 forecast 的算法、不再是 `ceiling` 的算法** —— 裁决见
  `docs/superpowers/specs/2026-09-17-t2-spec-amendments.md` §六。此后 `ceiling` 一律 =
  **整轮美元预算 ÷ 最贵费率**（即规格 §六 的口径，`50,505,050`），校准值只做披露、**永不传给
  `--ceiling`**。理由是实测的：Phase 2 的余量 `1,581,412` **小于**每题均值 `2,396,038`，
  第 10 题若跑起来必被截断 —— **把预测当上限，就必然在最后一题把自己掐掉**。
  **Task 15 / 16 / 17 的实际执行保持不变**，本条只说明这两处数字往后为何不再按原规则句解释，
  与本节第一条同性质：**留痕，不改断言**。
