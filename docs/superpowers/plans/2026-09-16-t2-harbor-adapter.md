...
4. **（真缺陷，非笔误；且它的「订正」本身还有第二层，到 Task 16 实跑才暴露）** Task 16 初稿写「verifier 在容器终态上重打 `test_patch`」—— 这是**已在另两处订正掉的假主张**（子仓 `CLAUDE.md`、`docs/claude-md-history.md`），并据此给 Task 16 补了 `declaredArtifacts` 这一项。**但那次订正只对了一半**：它把「官方 verifier 跑在**独立容器**、只读 `task.toml` 声明的 `artifacts`」当成了**无条件**的机制，而实情是**两种模式** —— 只有 `SEPARATE` 才如此，**默认是 `SHARED`**（`verifier_mode.py:10-26`、`trial/trial.py:667` 分派）。**Task 16 一跑就把这第二层打出来了**：本题与十题全体都是 `SHARED`、无一有 `artifacts` 键，门里据实记 `verdict: "N/A · 机制不适用"`（见该任务提交 `aee7dd6` 正文，它在提交时即按实测订正了 brief 的这句断言）。本计划上文三处与子仓 `CLAUDE.md` 2.48.0 行（`c8dc522`）现已一致。**留下的教训不是「要订正」，而是「订正时别把『我在一个对象上读到的』写成『全称』」** —— 上一轮把 `SEPARATE` 从「一种模式」升成了「机制本身」，于是「agent 改的东西能不能到 verifier」这条实测断言，在十道没有该机制题上被要求去验证一个不存在的东西。**救它的是设计而不是运气**：`declaredArtifacts` 是**逐字抄 `task.toml`**，抄到的就是「没有」—— **如实记「没有」也是读数**。
