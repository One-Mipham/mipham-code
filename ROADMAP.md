...
| 覆盖率          | 行 56.46% / 分支 47.34% / 函数 62.00% / 语句 56.19%（2026-09-15 实测）                                                | ⚠️ 偏低，**无阈值门禁，且 CI 不跑 `--coverage`** |
| 变异测试        | 无                                                                                                                    | ❌ 缺                                            |
...
**真实基线（2026-09-15 实测）**：语句 56.19%（9773/17390）· 分支 47.34%（4885/10317）·
函数 62.00%（1746/2816）· 行 56.46%（8951/15853）
...
第 1 步（并行；三者互为前提，构成闭环）
  T3a 覆盖率 include + 门禁  →  T1 遥测  →  T4 死代码盘点
     先看得见未加载文件  →  再看见真实调用  →  据此删除
...
**若只挑一件先做：`T3a`。** 半天成本、不依赖任何决策，而且是唯一一件「不做它，
后面几件的判断都缺依据」的事 —— 没有阈值与 CI 执行路径，覆盖率数字只会逐年漂移
而无人察觉。
