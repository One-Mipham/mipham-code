...
| 维度            | 现状                                                                                                                                                                                                                            | 判定                                                       |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| 测试            | 2547（2545 passed + 2 skipped），227 文件（另有 `apps/telemetry` 独立工作区 179 用例 / 12 文件）                                                                                                                                | ✅ 数量充足                                                |
...
| 文档数字守卫    | `test/integrity/` 5 个守卫文件（另有 1 个 fixture 文件不算在内）                                                                                                                                                                | ✅ 高于业界均值                                            |
...
第 3 步（补齐质量证据）
  T3b ✅ ESLint type-checked  →  T3c ✅ 变异测试（首批 6 文件）  →  T5 ✅ 未接线收口
     已落地（只开 no-floating-promises 一条，error 级）      已落地（基线 8.02%，待补断言）   已落地（第 3 步走完）

第 4 步（商业化）
  T6 模型端点 ∥ T8 桌面打包  →  T7 企业版
```

**`T3a`、`T1`（CLI 侧）、`T3b`、`T1b`（接收端上线）、`T3c`（首批）与 `T5` 已于 2026-09-15 落地**（见上）。
...
**下一件未阻塞的**（两件，都只欠一个判断、不欠前置）：
① **`T4` 的前三步**（knip 图分析 → 覆盖率 ∩ → `git log -S` 考古）**不需要数据窗口**，
只有第 4 步「用遥测投票」才需要 —— 而 T5 刚好清掉了三个未接线子系统里的一个同类项，
此刻正好是重跑前三步的时机（`rules-loader` 那种缺口已被守卫覆盖，重跑才有信息量）。
② **`T3c` 第二批（`crsi-sandbox.ts`）** 待单独决策：它每次踩到都要跑一遍整套套件，
成本与其余 6 个文件不同量级，要不要收、按什么顺序收，得单独拍。

**当前执行序（2026-09-15 定）**：`T3b` ✅ → **`T1b` ✅** → **`T3c` ✅（首批）** → **`T5` ✅** → `T2`（等决策）→ `T4`（前三步可动 / 第 4 步等数据窗口）——
