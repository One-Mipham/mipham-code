...
- [x] **D15** · **（2026-09-25 审公开面数字时发现；同日收口）i18n 的 vendored 复制对没有守卫，且 shared 副本已陈旧分叉**。
      **已收口的那半**：`web.features.slash_commands` 在**两份副本 × 两种语言**里都写「85 Slash Commands」/
      ...
      `bun -e` 真跑）⇒ **仓库里这两份副本**宣传的命令数是陈旧的（**订正见本条末**：这两份副本**并不是**
      线上 `mipham.ai/code` 的取数处 —— 那个 URL 302 到 `/mipham-code`、由 `websites/international`
      服务，其页面**硬编码 137**，线上从来没有因此错过）。
      ...
      **★ 订正（2026-09-25，浏览器实测；改掉本条目与同批变更记录里的一句假主张）**：原文写
      「**本仓库自己的产品页 `mipham.ai/code`**（`apps/web`）宣传的命令数是陈旧的」，并在变更记录里
      进一步写成「**未部署** ⇒ 线上 `mipham.ai/code` 仍显示 85，直到 `apps/web` 重新构建部署」。
      实测（本机 CDP 直连真 Chrome，三条路径各开一个后台 tab）：`https://mipham.ai/code`、
      `/code/docs`、`/code/install` **全部落到 `https://mipham.ai/mipham-code`**，落地页正文
      **逐字读出 `137 Slash Commands`**；国内站 `onemipham.com/mipham-code` 读出「137 命令」。
      两站正文里的测试数字是 **3473**。
      ⇒ ① **线上从来没有显示 85**，那句「直到 `apps/web` 重新构建部署」建立在一个**错的映射**上
      —— 我把「`apps/web` 里有 `/code` 路由」当成了「`mipham.ai/code` 就是它」，实测该路由在
      mipham.ai 上**被 302 遮蔽**、不是公开可达面（`mipham.ai/code*` 的公开服务方是 `websites`）；
      ② 本笔的 `85 → 137` 修的是**仓库里的两份副本**，该不该做是另一回事，但**不能用「线上会因此变对」
      当理由** —— 线上那两处 `137` 一直都在；③ 顺带量出**两站共 4 处**同类字面量：`137` 已对、
      **`3473` 陈旧**（命令产出的当下真值是 **3,486**）⇒ 后者才是 **D16** 的对象。
      **教训（与 [[claims-ahead-of-evidence]] 同族）**：「产品页 A 从文件 B 取数」这一步**我没有验**，
      是从仓内目录结构**推**出来的；**公开面必须实测落地 URL 与正文**，推出来的映射不算。
      **★ 收口（2026-09-25，裁定 = 删死命名空间 —— 「删键」是不可逆方向的选择，经用户裁定后执行）**：
      本条当初把落点写成「加进族表 + 按**共有键的值必须相等**比」。**那条判据是错的**，
      两个理由都落在实测上：① 施加到全量 586 键 ⇒ 那 8 条**死文案**要被永久按住同步 ——
      CLI 每改一次 hooks / 快捷键文案，就得手改一份**没人渲染**的 shared 副本；
      守卫的成本花在有消费者的地方才叫守卫，花在没消费者的地方叫**维护税**。
      ② 它**抓不到本笔真正的错值** —— `web.features.slash_commands` 曾写「85」（真值 137），
      而**两份当时写的是同一个错值** ⇒ 值相等的守卫只会全绿。抓到它的是**真值**判据
      （D16 落在 `published-counts.test.ts` 里那条）。**「两份相等」证的是两份一致，不是两份对。**
      **条目自己定位错了对象**：真缺口不是「两份可能漂」，而是**两份各带对方的死重**。
      命令量出的形状（改前读数取自 `HEAD`）—— shared **596** 键 = 92 个 `web.*` +
      **504 个非 `web.*`**（**零消费者**：`apps/web` 只读 `web.*`）；CLI **831** 键 = 745 个非 `web.*` +
      **86 个 `web.*`**（**CLI 一行都不读**）⇒ **590 / 1427 = 41% 是死的**。那 8 条值分歧
      正是长在这片没有消费者的地方，所以从来没人撞见。
      **故落点 = 按构造消除重复，而不是把旧关系按住**：两份副本各删掉对方的命名空间 ——
      shared **596 → 92**、CLI **831 → 745**，**共有键 586 → 0**（两种语言皆然）。
      此后「共有键值必须相等」**无对象可言**：不是被守卫按住，是**不存在了**。
      **替代守卫** `apps/cli/test/integrity/web-i18n-surface.test.ts`（新文件 / 7 条）：
      ① 把 `apps/web` 里**每个 `t(` 调用点逐个分类**取键 —— 引号、模板串（按同文件 `key:` 列表
      展开）、**认不出的形式进 `unhandled` 即红**。这一条是设计要害：用正则「抠键」的守卫有个
      致命退化 —— 换种调用写法就**静默不匹配** ⇒ 断言在越来越小的集合上恒真；
      ② 每个键必须在 shared 的**两种语言里都存在**（缺键 ⇒ 页面把原始 key 渲染上屏）；
      ③ 钉住分工本身（shared 只含 `web.*`、CLI 不含 `web.*`、共有键为 0）——
      守卫要钉的是一个**决定**，不是当下的数据。
      **四条负控逐条隔离、还原按 sha256 逐字核过**：A 从 shared 删 `web.dashboard.title` ⇒
      红且**点名该键**；B 往 `apps/web` 塞 `t(someDynamicKey)` ⇒ 红且点名文件:行；
      C 往 CLI 塞回 `web.stray` ⇒ 红；D 把扫描根指到文件数不足的目录 ⇒ **正对照自己红**
      （证明「扫到 0 个文件」不会静默全绿）。四条全咬到，收尾再跑回绿。
      **真构建 + 逐键读渲染**（不是只看构建绿）：`next build` 8 页静态生成 → `next start` →
      抓 4 个真路由（basePath `/code` ⇒ 真 URL 是**双层** `/code/code/*`）⇒ 抽取
      **84 键 / 78 个 `t(` 调用点 / 0 未分类**；**原始 key 上屏 0 处**；**76 键**的英文值在
      真渲染里逐字读到（探针按 HTML 实体还原 + 剥 React 的 `<!-- -->` 分段，否则插值文案假阴性）。
      剩下 **8 键不可能由抓页触发**，如实记：3 个 `web.error.*`（只在错误边界触发）、
      4 个 `web.not_found.*`（实测 Next 走的是**内建** 404、够不着 `apps/web/app/code/not-found.tsx`
      —— 线上同形：`mipham.ai/code/**` 的未知路径 302 走 `websites`；**这是既有路由事实、
      与删键无关**，未在本笔处置）、1 个 `web.models.status_upcoming`（三元分支的数据源里
      没有任何 `upcoming`）。⚠️ **一处巧合命中已识破**：`web.not_found.code` 的值恰是字符串
      `404`，它在 Next 内建 404 页里**偶然匹配上** ⇒ 探针一度报「77/84」，实为 **76**；
      **不核这一步就会把巧合当渲染**。
      **诚实边界（写下来，不假装覆盖）**：本守卫只保证**键存在**；`web.*` 里那些**非计数**的
      散文（如 `web.features.multi_model.desc`）**没有任何真值源**，写错了没有机器能发现 ——
      能立真源的只有计数，那半已由 D16 立了四个槽位。测试 3,494 → **3,501**（299 文件；
      integrity 14/97 → 15/104），活文档四处已在**同一提交内**回填。
      **披露**：`packages/shared/package.json` 的 `exports` 仍声明两个 locale JSON 路径，
      而该副本现在只装 `web.*` —— 本笔**未动** `package.json`（`npm view @mipham/shared` = E404，
      未发布 ⇒ 那两条 export 是接口意图、当前无外部消费者）。
...
- [x] **D16** · **收口（2026-09-25，`mipham-code` 与 `websites` 两仓 + 父仓 gitlink 均已推送）—— 两个官网的 `/mipham-code` 页各硬编码了一行测试数；修法是**立真源**，不是把手改做得更勤**。
      ...
      而真值 **2026-09-25 由命令产出为 3,486** ⇒ 两站同时陈旧（`git grep '3473'` 在 `websites` 里
      **实测恰好 2 处命中**，就是上面那两行；**线上也实测读出 3473**，见 D15 的订正）。
      **注**：本条原文此处写的「已变成 3480」**自己也在漂** —— 这正是本条的论点，故订正后只记
      「命令产出的当下真值 3,486」而不再在条目里保留中间值。
      另注（同一次实测）：两站的 `137` 同样**硬编码、同样没有真源** —— 只是它**此刻恰好是对的**；
      它与 `3473` 是同一个病，只差漂没漂。修的时候两处要一起搬进真源，否则下一次先漂的是 `137`。
      **已知边界**：① 它与 D15 在**不同仓库** —— `websites` 是 submodule，改动要单独提交 + 父仓 bump gitlink，
      不可能与本仓的改动同提交；② 两站的部署都是**显式命令**（国内 `deploy-cn.sh`、国际 `vercel deploy --prod`），
      仓库里只有 `ci.yml`、**没有部署 workflow** ⇒ 推代码本身不动线上，但「改完」到「上线」之间隔着一次显式部署，
...
- [x] **D17** · **收口（2026-09-25，未发布）—— `ChatRequest.signal` 有声明、有读者、有设置者，唯独没有「送达者」；而把送达补上之后，发现还缺第二半**。
      **原判**（写进 D8 的那句）是「接口级边界」，**实测不成立**：字段**早就有了**。`registry.ts:19` 声明 `signal?: AbortSignal`；
      `fetch-utils.ts:103` **确实会消费它**；`self-critique.ts:181`/`:187` **已经**在设它（`setTimeout(() => controller.abort(), this.config.timeoutMs)`
      配 `signal: controller.signal`，默认 **2000 ms**），`registry.chat:127` 用 `{ ...req, model: modelId }` 原样透传。
      断点只在**最末一跳**：`openai-compat.ts` / `anthropic.ts` 的 `fetchWithRetry` init **里没有 `signal` 字段** ⇒ `init.signal` 恒为 `undefined`
      ⇒ 那个 `AbortController` **一次都没接上过**，而 `abort()` 在无人监听的信号上是**空操作**。
      **所以这不是「缺一个字段」，是一条活缺陷**：`critique()` 的文档承诺「null if skipped or **timed out**」，**而那个预算从来没有生效过** ——
      真兜底的是 provider 自己的 `streamIdleTimeoutMs`（还按 `effort` 放大）⇒ 一次卡住的 critique 会把 `engine.ts:1219` 的 `await` 拖到几十秒。
      **① 送达**：两处 init 各加一行 `signal: req.signal`（`openai-compat.ts:44` / `anthropic.ts:137`）。
      **② 而只加这一行会把缺陷变得更坏 —— 这是原判漏掉的那一半。** `fetch-utils` 的 `finally` 里那个 `controller.abort()` 撤的是它
      **刚交给 `fetch`** 的组合信号，而调用方是在 `fetchWithRetry` **返回之后**才读 body 的 ⇒ **每一个流都会在响应头处被自己掐死**。
      **实测（Node v24.14.0 vs Bun 1.3.14，两个 throwaway 探针，同一份代码）**：**流中途** abort，两个运行时都让 body 报错；
      **首次 read 之前** abort —— **只有 Node 让 body 报错**，Bun 照旧把已缓冲的数据发出来 ⇒ **只在 Bun 上跑永远看不见这一格**。
      改法用 `AbortSignal.any([init.signal, controller.signal])`（Node ≥22 / Bun ≥1.2，均在 `engines` 内）：它对源信号持**弱引用**，
      组合在 body 读取期间保持活着、**又不需要任何清理** —— 手写的合并器做不到「既活着又不泄漏」（它唯一的清理手段就是 abort，
      而那正是掐死 body 的那一下；`dispose()` 版实现当场红在 `expected true, received false`，因为它在 `finally` 里跑、早于调用方的 abort，
      等于替 body 那段读取把线拔了）。`finally` 里**什么都不再撤销**，注释写清为什么。
      **判据被自己证伪过一次，留档**：第一版写的是「断言 `fetch` 收到了一个 signal」——**空判据**：`fetch-utils` 无论 `init.signal` 在不在
      都会塞一个自己的超时信号进去 ⇒ 负控跑红、这句却**照样为真**。改成暴露被捕获的那个信号、断言 **调用方 abort 之后 `probe.signal()?.aborted`
      为真**（那才是「撤销到达了传输层」本身；块数只是它的**结果**）。
      **负控三次，逐条隔离、还原按 sha256 逐字核过**：NC-1（openai 那行）⇒ 1 红；NC-2（anthropic 那行）⇒ 1 红；NC-3（②那半）⇒
      **恰好两条「不撤销」对照红、两条撤销用例保持绿**。四条最终用例落 `test/providers/stream-cancel.test.ts`（**7 → 11**；**订正 2026-09-25**：原写「11 → 15」，
      是把该文件的**战后**计数当成了战前计数 —— 由命令量得 `92f5763b` 上是 7、本笔之后是 11，与总数 3,486 → 3,490 的 +4 一致）。
      **默认值改成量出来的**（用户裁定「先量再定默认值」）：**30 次真 critique**（真模型真提示词，`findFastestModel` → **`deepseek-v4-flash`**，
      默认模型是更慢的 `-pro`）min **1.85s** / median **3.95s** / max **17.9s**，**28/30 超过 2s** ⇒ 那个 2000 ms 会让 **93% 的 critique 被静默跳过**；
      而 `critique()` 是 **fail-open**（超时 ⇒ `null` ⇒ 工具**未经审查**照跑）⇒ 表现正是「`/crsi critique` 看着是开的、其实什么都没做」。
      取 **15 s**（保住 27/30），仍远低于 provider 那条 90 s 流空闲兜底。**单模型、单日** —— 这个边界写在常量注释里。
      **两处被这次改动「连带变成假」的既有主张，同批订正**：① `self-critique.ts` 开头那句「a lightweight … check with **<200ms latency**」——
      实测中位数约 4 s，差 20 倍，改为「轻的是 prompt 不是时钟」；② `permission-classifier.ts` 里 `DEFAULT_CLASSIFIER_TIMEOUT_MS` 的
      「**Same bound as `self-critique.ts:52`**, the only measured precedent in this repo」—— 配对没了，判据也不再成立。
      **但分类器的 2000 不跟着改**：两者失败方向**相反**（它 fail-**closed**：超时对用户等同拒绝，预算由「一次合法调用可以被拒多久」界定，
      而 self-critique 的预算由「我们想让 critique 多常真的发生」界定）⇒ **兄弟的读数不能替代它自己的分布**。注释改成记下
      「一个兄弟读数现在存在，它是**不信任**这个 2s 的理由，而不是可以拿它替换的读数」—— 这是「搬运结论必须连限定条件一起搬」的反面用法。
      **测试 3,486 → 3,490**（providers 129 → **133**；文件数不变 297），CLAUDE.md **5 处同一提交内**回填、**只改数字**
      ⇒ `git diff --numstat -- CLAUDE.md` = **5/5**，且**每行字符数逐一相同**（无重排）、全文 **31,284 字符不变**。
      **一条读数注记（与本条无因果，但会让下一次归因走错）**：全量套件有**时序 flake**。本批三次全量跑：前两次分别红在
      `test/agent-view/dashboard-keys.test.ts`（2）与 `test/ui/input-autocomplete-wiring.test.ts`（4）——**两个文件都被本批逐字节未碰过**，
      单独跑 3 次 18/18 全绿；第三次 **297 文件 / 3488 passed + 2 skipped，全绿**。判据：**见红先单独跑那个文件再归因**，
      别把 297 worker 的调度抖动读成回归。
...
| 2026-09-25 | **D17 收口 —— `ChatRequest.signal` 有声明、有读者、有设置者，唯独没有「送达者」；把送达补上之后，发现还缺第二半**（未发布）。**原判「接口级边界」实测不成立**：字段早在 `registry.ts:19`、`fetch-utils.ts:103` 确实消费、`self-critique.ts:181`/`:187` 已经在设（配 2000 ms 预算）、`registry.chat:127` 原样透传 —— 断点只在**最末一跳**：两处 `fetchWithRetry` 的 init 里没有 `signal` ⇒ 那个 `AbortController` **一次都没接上过**（⇒ 文档承诺的「null if … timed out」从来没生效过）。**② 但只加那一行会让缺陷更坏**：`fetch-utils` 的 `finally` 撤的是它**刚交给 `fetch`** 的组合信号，而调用方是在**函数返回之后**才读 body 的 ⇒ 每一个流都会在响应头处被自己掐死。**实测（Node v24.14.0 vs Bun 1.3.14，同一份代码）**：**流中途** abort 两边都报错；**首次 read 之前** abort —— **只有 Node 报错**，Bun 照旧交付已缓冲数据 ⇒ **只在 Bun 上跑永远看不见**。改法 `AbortSignal.any`（对源信号持**弱引用**，组合在 body 读取期间活着且无需清理；手写合并器只能靠 abort 清理，那正是掐死 body 的那一下）。**判据被自己证伪过一次**：第一版「断言 `fetch` 收到了一个 signal」是**空判据**（`fetch-utils` 无论如何都会塞一个自己的超时信号）⇒ 改成断言**调用方 abort 之后那个信号的 `aborted` 为真**。**负控三次、逐条隔离、还原按 sha256 核过**：NC-1/NC-2 各 ⇒ 1 红，NC-3 ⇒ **恰好两条「不撤销」对照红、两条撤销用例绿**。**默认值改成量出来的**（用户裁定「先量再定默认值」）：**30 次真 critique**（真模型真提示词 → `deepseek-v4-flash`）min **1.85 s** / median **3.95 s** / max **17.9 s**，**28/30 超 2 s** ⇒ 那个 2000 ms 会让 **93% 的 critique 被静默跳过**，而 `critique()` 是 **fail-open**（超时 ⇒ `null` ⇒ 工具**未经审查**照跑）⇒ 表现正是「看着是开的、其实什么都没做」；取 **15 s**（保住 27/30）。**同批订正两处被这次改动连带变假的既有主张**（`self-critique.ts` 的「<200ms latency」；`permission-classifier.ts` 的「Same bound as `self-critique.ts:52`」）—— **但分类器的 2000 不跟着改**：两者失败方向**相反**（它 fail-**closed**），兄弟读数**不能替代**它自己的分布。**测试 3,486 → 3,490**（providers 129 → **133**），CLAUDE.md 5 处同提交回填、只改数字（numstat 5/5）。**读数注记**：全量套件有**时序 flake**（本批三次跑，前两次红在两个**未被本批碰过**的 UI 文件、单独跑全绿；第三次 297 文件 / 3488+2 **全绿**）⇒ **见红先单独跑那个文件再归因**。                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-09-26 | **0.85.6 发布 —— ROADMAP D8–D17 全部收口（23 笔，含 6 笔文档/杂务）**（2.94.6）。**① 更新面两笔，判据同一条：半截状态在构造上消失。** `mipham update` 的安装换成**原子换手** —— 装进 `<prefix>` **内部**的 staging（同一文件系统；落 `os.tmpdir()` 在 Linux 常因 tmpfs 报 `EXDEV` ⇒ 只能退化成拷贝 = 把要消灭的可中断中间态请回来）→ staging 上跑两道自证 → **两次 rename**（Node 没有 `renameat2(RENAME_EXCHANGE)`）→ 换手后再验一次 → 清理；旧方案的「写了一半」窗口是**分钟级且真实可达**，现在是一个 µs 级窗口。**回滚一律走反向 rename**，拷贝式快照整套退役；失败文案由 `rolledBack: boolean` 二态改四态（`untouched`/`restored`/`broken`/`unknown`）—— 最常见的失败是 staging 装不上，那时旧树**从未被碰过**，旧文案说「无法恢复原安装」是对用户机器的**假陈述**。Windows 那笔更重：npm 在 win32 下把全局包装进 `<prefix>/node_modules`（**没有 `lib` 层**），而旧代码把 Unix 的四层 `..` 用在两个平台 ⇒ prefix 多退一层、launcher 指向不存在的文件 ⇒ 自证第二道门（**真跑** launcher）必失败 ⇒ **每次 `mipham update` 都把刚装好的版本回滚掉，Windows 永远升不上去且每次都说「失败」**；同处还跳过了「绝不猜」那条对照物检查。它活这么久的原因就是形状本身：那半直接读 `process.platform`，在开发机与 CI（都是 Unix）上永远跑不到 ⇒ 平台改为**可注入参数**，两条布局分支在任何机器上都能真跑。**② provider 面两笔，都是「接线在场而世界不变」的极端形态。** `ChatRequest.signal` 有声明（`registry.ts:19`）、有消费者（`fetch-utils.ts:103`）、有设置者，**唯独没有送达者** ⇒ 那个 `AbortController` 一次都没接上过，`abort()` 在无人监听的信号上是空操作。而只把送达补上会让缺陷更坏：`fetch-utils` 的 `finally` 撤的是它刚交给 `fetch` 的组合信号，调用方却在函数**返回之后**才读 body ⇒ 每个流都会在响应头处被自己掐死（实测 Node 与 Bun 在这一格表现不同 ⇒ 只在 Bun 上跑看不见）⇒ 改 `AbortSignal.any`（对源信号持弱引用，组合在 body 读取期间保持活着、无需清理）。另一笔：两个 provider 的流式读循环**通篇没有 `reader.cancel()`**，而消费方确实会中途离开（引擎收到 `stop` 即 break、子代理 abort 时抛）⇒ 把**读循环连同兜底的 `stop`** 一起包进 `try/finally`（逐出口补覆盖不了「消费方半途走人」，而那正是这条路日常的形态）。**③ CLI 面六笔**：`--provider`/`--model` 顶层 flag 落地（障碍不在解析而在**次序** —— 值落进位置参数、撞上跑在所有 flag 解析**之前**的未知命令分支；两个**已发布** IDE 插件的启动命令此前是 `Unknown command` rc=1）；ghost-text 补全上行无上限（6 条 × 任意长的 `content` ⇒ 贴一个文件就是上万 token）、下行取消不掉（判据在循环外 = 先把整条流读完再丢结果）；`--resume` 后估值被单条系统提示覆盖（`needsCompaction()` 因此长期偏 false）；子代理声明的 `memory` **从没到过模型手里**（拼接挂在了没人读的那份上）；页脚把启动那一拍的 graft 状态冻死（`useState` 初始化只跑一次，而文件是**别人在后台写的** —— 且同文件注释把这个冻结**当成设计**写下来了，注释替缺陷背了书）；读侧不验形 5 处（「合法 JSON、错的形状」不抛 ⇒ `catch` 兜不住、也没人说话：梦历史把 `{"a":1}` 当数组、签名库躺着没有 id 的成员、台账拿下标当键、`new Set("bc")` **按字符**拆链接、召回统计一条脏记录赔上后面所有条目）。**④ daemon 面两笔**：`daemon.log` 起手轮转（mkdir 之后、open **之前** —— 唯一没有写者的时刻；**改名而非截断**，只留一代 ⇒ 上限恒 2 × 5 MiB；同目录 `permission-audit.jsonl` **刻意不动** —— 审计台账静默丢行是策略决定）；smoke 就绪检查由 `grep -q` 改捕获式（`pipefail` 下生产者的 SIGPIPE(141) 被读作「无匹配」⇒ 删除侧会把活着的 daemon 读成已停），两条守卫分支**此前零执行**、首次真跑证明它们本来就对，超时分支另修真缺陷（放弃前再探一次 + 回收自己起的子进程）。**⑤ 公开面两笔**：产品页页脚那个测试数被**手改过至少 7 次**（2262→…→3473，而最后一次改动当天真值已是 3,486 ⇒ 手改版数字的期望寿命比两次发布之间的间隔还短）⇒ `package-info.json` 立四个计数槽 + 三处落盘副本 + 三道机器判据（树守卫守**一致**、CI 硬门禁守**对不对**）；两份 i18n 副本各带对方的死重（590/1427 = **41% 死**）⇒ 各删掉对方的死命名空间、重复**按构造消失**（596→92 / 831→745 / **共有键 586→0**），并换上**真判据**的守卫 —— 旧判据「共有键的值必须相等」**是错的**：那 4 个文件曾**同时**写着 `85`、真值 137 ⇒ 值相等只会全绿，「两份相等」证的是两份**一致**、不是两份**对**。测试 3,426 → **3,506**（291 → 299 文件，0 失败）；子仓 2.94.5 / npm 0.85.5 均未变 ⇒ 本批此后才发布。 |
