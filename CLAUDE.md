...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.56.0
> **最后更新**: 2026-09-18 — **两条真缺口落地（a `/resume` 健壮性 / b 三个状态文件原子写）**：(a) `SessionLog.open()` 是磁盘→内存**唯一**入口却只 `JSON.parse` 不校验 —— 合法 JSON ≠ 合法事件，`compaction/rewrite` 少 `messages` 进内存后投影即抛，**一个字节毁掉整份会话**；加 `isValidEvent` 只校验**投影真有分支**的变体、未知 `type` 放行。`SessionStore.list()` 的 `try` 包住整个 `for` ⇒ 一个文件读不出来就让 `/resume` 空列表，改逐文件兜底。两处失效面不相交，用负控钉死（撤逐文件 try **只 1 红**）。(b) `preferences.ts` / `keys-manager.saveKeys` / `tools/system/config.ts` 三处裸写**原地截断**，而三个读侧都把「读不出来」吞成空 ⇒ 丢的是**整份**；改原子写 + 0o600（`saveKeys` 原先是「转了一半」：写固定名 `.tmp` 却不 rename）。**判据取 inode 不取权限** —— 0o600 与「不留 `.tmp`」裸写同样满足，先实测判别式再写进四条用例。测试 2645 → 2661（230 → 231 文件）
> **前一条（#12 后半）**: 2026-09-18 — **#12 后半：删掉并行版本机制 + 三条就地修**：两个版本机制只有 manifest 归档活在链路上，另一个（`versioning.ts`）的唯一写者 `saveVersion` **零调用点** —— 「恢复上一版」从未被喂过、它维护的 `current.html` 从未存在过 ⇒ 删除，`/artifact` 按名字订阅改读工具**真正写下的那份文件**、找不到即 404。删除当场暴露一个真缺陷：`stop()` 只调 `server.close()`（只停止**接收新连接**，已建立的 socket 继续被服务）⇒「已停止」的服务器仍在旧端口应答，而它的目录已被测试删掉 —— 新 SSE 用例单跑绿、全量约 3/8 红。另三条执行中撞见：manifest 去重只按 `name`（全局一份索引，两会话同名互相顶掉）、读失败静默当空而写路径整份重写（坏索引被覆盖成「只有这一条」）⇒ 读写分两条路径 + 隔离坏索引 + 原子写；无 `cwd` 的旧 cron 任务仍照发（刻意）⇒ 列表标 `[无归属]` 并给出处置办法。测试 2658 → 2645（231 → 230 文件）
...
测试：2,661 测试（2659 passed + 2 skipped，0 失败）
...
| --------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| core            | 72      | 1068     | engine / context / permission / hooks / crsi / memory / instructions / paths 等                                                                                             |
...
| config          | 6       | 35       | credential-crypto / loader-encryption / defaults / settings-json / preferences                                                                                              |
...
| **合计**        | **231** | **2661** | **0 失败** ✅（2659 passed + 2 skipped）                                                                                                                                    |
