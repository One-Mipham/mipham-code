...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.4
> **最后更新**: 2026-09-14 — daemon 外部 API 修「任意网页可驱动 agent 读全盘」（新增 `originMiddleware` 校验 `Origin` + `POST /api/v1/sessions` 的 `cwd` 限信任子树）；内置 `superpower` skill 2.0.0 → 2.1.0；测试数回填 2280 → 2293（daemon +1 文件 / +13 测试）；v0.81.5 发版（跳过 0.81.4 —— 该号被 VS Code 扩展的 changelog-only 重发占用）
...
测试：2,293 测试（2291 passed + 2 skipped）
...
| **合计**        | **208** | **2293** | **0 失败** ✅（2291 passed + 2 skipped）                                            |
...
> **完整修订历史**（v1.0.0–v2.37.4，共 84 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
