...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.53.0
> **最后更新**: 2026-09-18 — **搜索工具的截断与错误分类（Phase 2）**：`rg` 快路径此前**完全不走**输出上限（同一条函数里只有 find 回退截断）；find 回退把「根本没搜成」读成「无匹配」（BSD find 对不可读目录/非法正则/缺 grep **都退 1**，只有退出码可看）；两条路径的 stderr 全程无人读 —— 顺序读 stdout 再读 stderr 会在 stderr 写满 ~64 KB 管道缓冲时**真卡死**（实测 5 s 超时兜底）。三条各自配负对照。测试 2633 → 2639（tools 328 → 334）
...
测试：2,639 测试（2637 passed + 2 skipped，0 失败）
...
| tools           | 20      | 334      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
...
| **合计**        | **228** | **2639** | **0 失败** ✅（2637 passed + 2 skipped）                                                                                                                                    |
...
> **完整修订历史**（v1.0.0–v2.53.0，共 105 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)：
