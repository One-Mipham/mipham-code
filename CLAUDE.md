...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.14.0
> **最后更新**: 2026-08-26 — P1 三连（客户端错误渲染成错误行 #23 / 沙箱违规 exit-0 回归 #35 / 富权限错误 #52）+ 测试数对齐 1821
...
测试：1,821 测试（1819 passed + 2 skipped）
...
| **合计** | **166** | **1821** | **0 失败** ✅（1819 passed + 2 skipped）      |
...
| 版本   | 日期       | 变更内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 维护人     |
| ------ | ---------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| 2.14.0 | 2026-08-26 | P1 三连（CC 2.1.243 借鉴落地）：① **#23 客户端错误渲染成错误行** — `engine.process` 遇 `chunk.type==='error'` 时把错误存为 `role:'assistant'`（模型输出），resume 时误渲染成 ◆ assistant；改存 `role:'system'`（⚠ 错误行），与 UI 已有的 `❌ Error` 系统行一致 ② **#35 沙箱违规 exit-0 丢失** — 核查 `bash.ts` 的 `detectViolations` exit-0 路径已正确拼接违规详情（非真缺口），补 2 条集成回归测试锁死「exit 0 + stderr 违规 → content 含 Sandbox Violations」 ③ **#52 富权限错误** — 新增 `PermissionSystem.explainDenial()`（deny-rule/ask-rule/legacy-rule/mode-baseline/tool-default/system-default 六因），`engine.buildDenialError` 按因点名 mode + setting + 修复：deny-rule「拒绝规则优先于模式，改用其他方式」、其余「Shift+Tab 或 /permissions」。+6 测试（2 bash + 4 explainDenial）。测试 1815→1821 passed（1819 passed + 2 skipped，166 文件）。 | 技术委员会 |
