...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.54.0
> **最后更新**: 2026-09-18 — **数据落点与并发写（Phase 3）**：五条同形 —— 同一个事实被两处各自算一遍。artifact 的目录（工具写 `<cwd>/artifacts`、服务端根在 `<cwd>/.mipham/artifacts` ⇒ 工具印的 URL **必然 404**）收进 `artifactsRoot()`；`archiveVersion` 源文件不在时照样推进版本号（manifest 里留一版磁盘上没有的）；`atomicWriteFileSync` 的固定 `.tmp` 名在共享路径上会**丢写**；cron 的 prompt 塞进**当前会话**执行 ⇒ 加 `cwd` 归属并按 cwd 过滤；Read 声称支持 offset/limit 却先整读再切片（超 50 MB 一律报错，而错误建议的「use offset/limit」**不可执行**）。四条负对照各自只咬一条，其中两条最初没咬住是测试自己的问题（边界不可达 / 凭据规则拼的是原始 tmpdir 而函数返回 realpath）。测试 2639 → 2658（228 → 231 文件）
> **前一条（Phase 2）**: 2026-09-18 — **搜索工具的截断与错误分类（Phase 2）**：`rg` 快路径此前**完全不走**输出上限（同一条函数里只有 find 回退截断）；find 回退把「根本没搜成」读成「无匹配」（BSD find 对不可读目录/非法正则/缺 grep **都退 1**，只有退出码可看）；两条路径的 stderr 全程无人读 —— 顺序读 stdout 再读 stderr 会在 stderr 写满 ~64 KB 管道缓冲时**真卡死**（实测 5 s 超时兜底）。三条各自配负对照。测试 2633 → 2639（tools 328 → 334）
...
测试：2,658 测试（2656 passed + 2 skipped，0 失败）
...
| tools           | 21      | 345      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
...
| **合计**        | **231** | **2658** | **0 失败** ✅（2656 passed + 2 skipped）                                                                                                                                    |
...
| 2.52.0 | 2026-09-18 | **插件安装不再执行被装包的脚本（清单 #27，提前于 Phase 2，`6533d40`）** —— 27 条里**唯一**「一条命令拿到全权执行」的路径：`installFromNpm` 用 `execSync('npm install <pkg> … --no-save')`，缺 `--ignore-scripts` ⇒ 被装包的 `postinstall` 以当前用户全权跑一遍；`/install-plugin` 可达。**为什么是缺陷**：插件在这里只需**是**磁盘上的文件（`install(sourcePath)` 那条路径就是纯拷贝 + 清单校验），装它不需要跑它的构建 —— 拿全权执行换一个用不上的能力是净损失。**两处改动同一行**：①加 `--ignore-scripts`；②`execSync` 模板串 → `execFileSync(argv 数组)`。**②的诚实边界**：包名白名单本就挡住 shell 元字符，**故不声称修掉了一个已可达的注入**，它只是让那条白名单不再是「我们与 shell 之间」唯一的一道。**证据分两半分别测**（不合并成一句「已验证」）：本机 npm 11.9.0 真探针 —— 带 `postinstall` 的 fixture 包不传 flag 时标记文件**真被写出**、传了则**不存在**（证 flag 有效）；单测断言我们**真的传了**它（证用的是它）。**负控**：摘掉 flag ⇒ **恰好 1 条红**（就是那条断言），其余 18 条仍绿。顺带：该测试文件此前对 `installFromNpm` **零覆盖**，新增 4 条接上。测试 2629 → 2633（plugin 28 → 32）。**本条为摘要，全文见 [history.md](docs/claude-md-history.md)。**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |

> **本表只留最近 3 行**（滚动窗口，同上方约定）—— 上表是**摘要**，每条全文与被挤掉的行都在 history.md。
> **完整修订历史**（v1.0.0–v2.54.0，共 106 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)：
