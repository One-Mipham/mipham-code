...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.63.0
> **最后更新**: 2026-09-19 — **定向安全审计 N1：项目级 hooks 在问不出信任的地方照样会执行** —— 形状是**「守护点的作用域小于被守护物」**：`<cwd>/.mipham/settings.json` 的 `hooks` 是仓库可控、本进程会 `spawnSync` 起的 shell 命令，而信任闸只在交互式 TTY 上问（无 TTY 直接 return），daemon 这条路径**从不调用它** —— 而 `isCwdAllowed` 只要求会话 cwd 在 daemon 根之内（`isWithin(cwd, daemonRoot)` 或 `isTrusted(cwd)` 二者其一），故未信任的克隆天然满足、可让本进程照其配置起子进程。① 闸从「进程级一次、只在 TTY」下移到**加载点** —— `loadSettingsJson(cwd, {includeProjectHooks})` **默认 false**（读项目 hooks 重新定义成一次信任行为），三处调用点各自表态：`index.tsx` 与 daemon `hooksFor` 按 `isTrusted()` 判定，`/hooks` 显式传 `true`（它只是**显示**，且 TUI 只在信任落定后才启动）；`permissions` 不受该开关影响（那是模式上限的问题，不是信任的问题）。② hook 子进程的 env 一并收口：裸 `spawnSync` 把整个环境交给 hook，改用 Bash 自 E1 起就在用的同一条 `filterEnv` 掩码 —— 只取用户级配置（`HookContext` 不带 cwd，无会话作用域可读项目段）。**判据不许宣布一次没发生过的跳过**：新增 `projectHooksSkipped`，由**同一份解析**回答「到底扣下了没有」，故告警不可能跑在事实前面 —— 第一版直接在调用点写警告，全绿那一跑打出 **8 条**（含无 settings.json 的工作区），改后收敛到**恰好 1 条**，并补用例钉住「没有项目 hooks 就不许告警」的反方向。三条负控互不相同：`readHooks` 恒 `true` ⇒ 3 红、无条件报告跳过 ⇒ 1 红、摘掉 `env` 传参 ⇒ 1 红。测试 2800 → 2809（2807 passed + 2 skipped，244 文件不变）。
> **前一条（2.62.0）**: 2026-09-19 — **定向安全审计批次 4：P 族六条（`8c4dc128`→`9170fae2`，P1–P6）**：六条全在权限层，**失效方向一致地偏宽** —— 每一条都是「声明的上限没够到真正的执行点」。① **写错的 `permissionRestrictions` 静默失效**（档名拼错、类型给成数组、`maxAllowedMode` 填未知档）⇒ 组织策略一个字都不生效且零告警；改为告警 + fail-closed 钉到最严一档。② **上限管不住 allow 规则通道**：`check()` 第 3 步 allow 命中直接返回 `'bypass'`，**加了上限反而能靠一条 allow 规则绕过去**（规则比上限宽）；改为 allow 命中后再过一遍上限。③④ **层级表两处把宽严判反** ⇒ `maxAllowedMode` 放行了比它更宽的一档。⑤ **页脚与系统提示读的是「近似的替身」**：页脚读本地 `useState`（初始写死 `default`、Shift+Tab 存的是**请求值**），系统提示读 `config.permission` 原始值（非法拼写时那一段**整段消失**）—— 两处都不等于引擎真正所在的模式，于是「启动即 `bypass`、页脚显示 default」可以长期共存；改为把权限系统**建一次**交给引擎、两处回读同一个实例。⑥ **子代理没拿到权限系统时整道审批闸消失**：`subPermission?.needsApproval(...)` 的可选链短路成 `undefined`，工具照跑 —— 而紧挨着的上一行正断言「需要审批的工具要拒绝」；改为缺席即按 CLI 自己的 `default` 档检查（负控实测：档位选 `bypassPermissions` 只 1 红、选 `plan` 会红 6 条 ⇒ `default` 是能堵上缺口的最小档位）。测试 2767 → 2800（241 → 244 文件）。
...
测试：2,809 测试（2807 passed + 2 skipped，0 失败）
...
| --------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| core            | 73      | 1114     | engine / context / permission / hooks / crsi / memory / instructions / paths 等                                                                                             |
| tools           | 25      | 383      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
| daemon          | 34      | 211      | feishu / telegram / 钉钉 / 企业微信渠道 + session / auth / auth-rotate / workspace-guard / logger + **引擎接线行为**（`engine-capabilities`）                               |
...
| **合计**        | **244** | **2809** | **0 失败** ✅（2807 passed + 2 skipped）                                                                                                                                    |
...
| 2.63.0 | 2026-09-19 | **定向安全审计 N1：项目级 hooks 在问不出信任的地方照样会执行** —— 形状是**「守护点的作用域小于被守护物」**：`<cwd>/.mipham/settings.json` 的 `hooks` 是仓库可控、本进程会 `spawnSync` 起的 shell 命令，而信任闸只在交互式 TTY 上问（无 TTY 直接 return），daemon 这条路径**从不调用它** —— 而 `isCwdAllowed` 只要求会话 cwd 在 daemon 根之内（`isWithin(cwd, daemonRoot)` 或 `isTrusted(cwd)` 二者其一）⇒ 未信任的克隆天然满足，clone 一个不可信仓库即可让它照着自己的配置起子进程。① 闸从「进程级一次、只在 TTY」下移到**加载点** —— `loadSettingsJson(cwd, {includeProjectHooks})` **默认 false**（读项目 hooks 重新定义成一次信任行为），三处调用点各自表态：`index.tsx` 与 daemon `hooksFor` 按 `isTrusted()` 判定，`/hooks` 显式传 `true`（它只是**显示**，且 TUI 只在信任问题落定之后才启动）；`permissions` 不受该开关影响（那是模式上限的问题，不是信任的问题）。② hook 子进程的 env 一并收口：裸 `spawnSync` 把整个环境（含全部 provider key / bot secret）交给 hook，改用 Bash 自 E1 起就在用的同一条 `filterEnv` 掩码 —— 只取用户级配置（`HookContext` 不带 cwd，无会话作用域可读项目段）。**判据不许宣布一次没发生过的跳过**：新增 `projectHooksSkipped`，由**同一份解析**回答「到底扣下了没有」，故告警不可能跑在事实前面 —— 第一版直接在调用点写警告，全绿那一跑打出 **8 条**（含根本没有 settings.json 的工作区），改后收敛到**恰好 1 条**，并补用例钉住「没有项目 hooks 就不许告警」的反方向。三条负控互不相同：`readHooks` 恒 `true` ⇒ 3 红、无条件报告跳过 ⇒ 1 红、摘掉 `env` 传参 ⇒ 1 红。测试 2800 → 2809（2807 passed + 2 skipped，244 文件不变）。                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | 技术委员会 |
