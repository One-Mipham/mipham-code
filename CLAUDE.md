...
> **产品**: 多模型开源智能编程终端
> **版本**: 1.9.0
> **最后更新**: 2026-08-11 — 下一步计划新增 Daemon 后台持久化架构
...
7. **Daemon 后台持久化** — 终端断开后 Agent 会话持续运行，支持无缝重连（`mipham attach`）
   - 借鉴 [prime-agent](https://github.com/PrimeIntellect-ai/prime-agent) 的 daemon 架构
   - Session worker 独立进程，生命周期与终端解耦
   - 心跳检测 + 自动 compact + 持久化 goals
   - 安全约束：worker 继承 Mipham Code 的 6 级权限系统，不引入 IPython 自由执行模型
