...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.3
> **最后更新**: 2026-09-14 — 接线 `core/rules-loader.ts`（补 `index.tsx` 注册 + `continueWithTools` 第二处注入路径，模块行覆盖 0% → 98.07%）；覆盖率实测（`pnpm coverage`，行 55.1% / 分支 45.64%）+ 核实 knip 清单与覆盖率 0% 清单**不相交**；v0.81.2 发版
