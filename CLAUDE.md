...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.7
> **最后更新**: 2026-09-15 — 七项修复批次 + 新增纪律「文档任何时刻正确」（凡改变被测数量的提交**须在同一提交内**回填活文档数字）：① 活文档事实校正（`apps/cli/README.md` 版本号与三处数字、根 README 工具数、PRODUCT.md 标明为定格快照）② `bash -c '…'` 绕过 `Read()` deny —— shell `-c` 的 payload 作嵌套命令行递归解析 ③ workflow 脚本目录读写归一（`core/paths.ts` 新增 `workflowScriptDir`/`workflowScriptDirs`）④ `MIPHAM_DIR` 同名不同义拆为 `MIPHAM_HOME` ⑤ 修订历史成文「5 行滚动窗口」+ 守卫 + 旧条目搬运（CLAUDE.md 56,001 → 25,810 字符）；测试 2361 → 2378（212 文件不变）
...
| 2.37.5 | 2026-09-15 | 三处修复：`READER_COMMANDS` 补 20 条「读文件写 stdout」命令（此前 `fmt secret` 绕过 `Read(secret)` deny）；MCP `tools/list_changed` 由逐条全量重注册改为去抖 250ms + 上限 2000ms 合并刷新；`/clear` 与 `/resume` 补清 `engine.readFiles`（会话级「读过才能覆盖」凭据）。三处均先红后绿。测试 2293 → 2304。                                                                                                                    | 技术委员会 |
| 2.37.4 | 2026-09-14 | daemon 外部 API 安全修复：新增 `originMiddleware`（有 `Origin` 且不在白名单即 403）与 workspace-guard 的 `isCwdAllowed`（拒绝在未信任 workspace 外开会话）。此前 auth 按 socket IP 判 loopback 免鉴权、CORS 只护响应头，两者都只防「响应被读到」不防「请求被发出」。测试回填 2280 → 2293。                                                                                                                                    | 技术委员会 |
| 2.37.3 | 2026-09-14 | 接线 `core/rules-loader.ts`（自 e2be832 起只有定义没有接线，模块从不加载）；补 `continueWithTools` 多轮循环的第二处注入点 —— 只接首轮即本仓库已固化的「两条路径只接一条」失败模式。行覆盖 0% → 98.07%，测试 2266 → 2280。                                                                                                                                                                                                     | 技术委员会 |

> **本表只留最近 5 行**（滚动窗口，见上方 `## 最近提交` 的同名约定）—— 上表列的是**摘要**，
> 被挤掉的行与每条的全文本都在 history.md，逐字未删。
>
> **完整修订历史**（v1.0.0–v2.37.7，共 87 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
