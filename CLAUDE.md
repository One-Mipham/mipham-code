...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.3
> **最后更新**: 2026-09-14 — 接线 `core/rules-loader.ts`（补 `index.tsx` 注册 + `continueWithTools` 第二处注入路径，模块行覆盖 0% → 98.07%）；覆盖率实测（`pnpm coverage`，行 55.1% / 分支 45.64%）+ 核实 knip 清单与覆盖率 0% 清单**不相交**；v0.81.3 发版（patch —— 修「声称有但不生效」的能力）
...
测试：2,280 测试（2278 passed + 2 skipped）
...
| --------------- | ------- | -------- | -------------------------------------------------------------------------- |
| core            | 71      | 969      | engine / context / permission / hooks / crsi / memory / instructions 等    |
...
| **合计**        | **207** | **2280** | **0 失败** ✅（2278 passed + 2 skipped）                                   |
...
> **完整修订历史**（v1.0.0–v2.37.3，共 83 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
