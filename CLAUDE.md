...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.46.0
> **最后更新**: 2026-09-15 — **T3c 第二批**：`crsi-sandbox.ts` 接入 `mutate`（**7 文件全接**），基线 **7.05%**；延后它的理由**被实测证伪**（`runTests()` 零测试覆盖 ⇒ 其变异体全落 `NoCoverage`，Stryker 对它们什么都不跑），并附一次对照实验：同 6 文件范围复跑 killed **130 → 137** ⇒ 同配置噪声是原记「±1」的 7 倍。测试 2512（227 文件不变）
...
>
> **完整修订历史**（v1.0.0–v2.46.0，共 98 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
