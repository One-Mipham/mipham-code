...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.58.0
> **最后更新**: 2026-09-19 — **加固计划 Phase 5（缺口 #24–#26，三条体验项）**：三条同源 —— **工具报给模型的「事实」不完整、或指向错误的根因**。#24 Glob 撞 500 上限直接 `break`，一个字都不说（同一份代码里 Grep 特意加了 `(truncated)`，理由还写在注释里）⇒ 模型把「只看到 500 条」当成「一共 500 条」；改为**先探到第 501 个**再置截断标记（恰好 500 个是完整结果，不谎报），注解在掩码**之后**拼接（`maskGlobOutput` 逐行当路径比对，注解不是路径）。#25 `catch {}` 把 rg 的任何异常都说成「没装 ripgrep」⇒ 真的失败（EAGAIN/EPERM）先静默回退到 `find`，再由回退那侧的 catch 报出「Install ripgrep」，排查方向整条带偏；改为只有 `ENOENT` 才回退，其余原样报 `code: message`，回退自身起不来时同样报真实原因。#26 Edit 审批预览 `slice(0,60)` 按 **UTF-16 码元**切 ⇒ 落点夹在代理对中间就留下半个 emoji（渲染成 U+FFFD）；抽出 `truncateForDisplay` 退到码点边界，同函数另两处预览（Agent 描述 / 未知工具 JSON）用同一规则。测试 2693 → 2702（232 → 233 文件），`/crsi eval` 100/38 不退化。
> **前一条（2.57.0）**: 2026-09-19 — **加固计划 Phase 4（缺口 #17–#23，五笔 `b89765f`→`e2aed83`）**：七条一个共同形状 —— **定义在那儿，施加点没接上**：#17 屏蔽目录存字面量而检查拿 `realpathSync` 后的路径去比（macOS `/etc`→`/private/etc`，**本机是死的**）；#18 配置合并太浅**整片打掉兄弟键**；#19 `.mcp.json` 手写重建条目丢 `request_timeout_ms`/`auth`；#20 Stop 的拒绝断在 `runHooks` ⇒ 出口统一派生；#21 工具 matcher 装成正则却按精确相等过滤 ⇒ 一直是空的；#22 子代理 ctx 无 `permissionSystem` ⇒ 嵌套工具调用**零审批**；#23 HTTP 通知从不派发 + `reconnect()` 零调用点 ⇒ 断连后 `/mcp` 仍是绿的。测试 2661 → 2693（231 → 232 文件）。
...
测试：2,702 测试（2700 passed + 2 skipped，0 失败）
...
| tools           | 21      | 355      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
...
| **合计**        | **233** | **2702** | **0 失败** ✅（2700 passed + 2 skipped）                                                                                                                                    |
...
| 版本   | 日期       | 变更内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 维护人     |
| ------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| 2.58.0 | 2026-09-19 | **加固计划 Phase 5（缺口 #24–#26，三条体验项，一笔 `c6c0edb`）** —— 三条同源：**工具报给模型的「事实」不完整、或指向错误的根因**。① Glob 的 500 上限**静默截断**（同一份代码里 Grep 特意加了 `(truncated)` 标记，理由还写在注释里）⇒ 模型把「只看到 500 条」当成「一共 500 条」；改为**先探到第 501 个**才置截断标记（恰好 500 个是完整结果，不谎报），注解在掩码**之后**拼接（`maskGlobOutput` 逐行当路径比对，注解不是路径）。② Grep 的 `catch {}` 把**任何**异常都说成「rg 没装」⇒ 装了却起不来的（EAGAIN/EPERM）先静默回退到 `find`，再由回退那侧的 catch 报出「Install ripgrep」，排查方向整条带偏；改为只有 `ENOENT`（或等价文案）才回退，其余原样报 `code: message`，回退自身起不来时同样报真实原因。③ Edit 审批预览 `slice(0,60)` 按 **UTF-16 码元**切，落点夹在代理对中间就留下半个 emoji（终端渲染成 U+FFFD）；抽出 `truncateForDisplay` 退到码点边界，同函数另两处预览（Agent 描述 / 未知工具 JSON）用同一规则。测试 2693 → 2702（232 → 233 文件）；每条先跑红再实现，③ 另跑负控（去掉边界回退 ⇒ 恰好 3 条红，两条反方向用例保持绿）。**本条为摘要，全文见 [history.md](docs/claude-md-history.md)。**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | 技术委员会 |
