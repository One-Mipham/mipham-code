...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.65.0
> **最后更新**: 2026-09-19 — **观察③：`/hooks` 列出项目级 hooks，却不标注它们已被信任闸扣下** —— 合并后的列表**不带来源**：项目级与用户级的条目躺在同一个 bucket 里，而两者**受不同管辖**（只有项目级那条过 `isTrusted` 闸）。显示命令为了不藏东西显式要了项目文件 ⇒ 闸关着时它照样列出、且不置一词。修法是**在加载处留住来源**，不是在显示处重解析：`SettingsJson` 增 `projectHooks`（沿用 `projectHooksSkipped` 那条「只在真发生时才出现」的规则），`/hooks` 分两次取（不带选项 = 用户级，带选项 = 项目级），逐行标 `[project]`/`[user]`，闸关时给项目级行追加「不会运行」；受信时输出与从前**逐字相同**（顺序也照旧：同一事件内项目级在前）。**判据两条轴**：标注跟**来源**走、闸只管**标不标**。测试 2814 → 2821（245 文件）。
> **前一条（2.64.0）**: 2026-09-19 — **hook 的「在哪儿」取的是 `process.cwd()`，不是会话 cwd**（观察②，E8 同族）—— `executeCommand` 用它同时回答两个问题，而两个都不属于本进程：hook **在哪个目录跑**（`spawnSync` 不给 `cwd` 即继承）与它**被告知**自己在哪（stdin 的 `cwd` 字段）。一次性 CLI 恰好都对；daemon 一个进程服务多个会话，`process.cwd()` 不属于任何会话 ⇒ 每个 daemon hook 都落在那里跑、并读到那里。修法是**接线**：`HookContext` 增可选 `cwd`，由 `HookEngine` 在 `runHooks` **一处**盖章。**判据是「两半都断言」**（跑在哪 + 被告知在哪）；三条负控**红集互不相同**（2 / 3 / 1 红）。**未修观察**：hook 掩码仍取用户级而 CLI 的 Bash 拿项目级 —— 属**策略**改动，另案。测试 2809 → 2814（2812 passed + 2 skipped，244 文件不变）。
...
测试：2,821 测试（2819 passed + 2 skipped，0 失败）
...
| **合计**        | **245** | **2821** | **0 失败** ✅（2819 passed + 2 skipped）                                                                                                                                    |
