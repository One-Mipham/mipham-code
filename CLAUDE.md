...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.48.0
> **最后更新**: 2026-09-16 — **工具成败位在无头路径上不可读（T12 A 段）**：`StreamChunk` 没有能装工具成败位的字段，`ServerToolResultMessage.isError` 声明了却从未被填；同时挖出 `continueWithTools()` 未做 flatten，失败工具的 `error` 文案在多轮循环里整个丢失（模型收到一个**空** tool_result）。两处统一补齐，WS 去程与回程都带。测试 2516 → 2521
...
测试：2,521 测试（2519 passed + 2 skipped，0 失败）
...
| --------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| core            | 72      | 999      | engine / context / permission / hooks / crsi / memory / instructions / paths 等                                                                                             |
| tools           | 20      | 313      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
| daemon          | 32      | 179      | feishu / telegram / 钉钉 / 企业微信渠道 + session / auth / workspace-guard / logger + **引擎接线行为**（`engine-capabilities`）                                             |
...
| **合计**        | **227** | **2521** | **0 失败** ✅（2519 passed + 2 skipped）                                                                                                                                    |
...
| 2.47.0 | 2026-09-15 | **daemon 无头路径修复：`stop` 上的 `break` 丢掉了整个工具循环** —— `session-worker.processPrompt()` 在 `chunk.type === 'stop'` 时 `break`，但该类型在消费端**被重载**：provider 侧的 A 类（`openai-compat.ts:182` / `anthropic.ts:245`，每条 LLM 流结束时**无条件**发出、含工具调用那一轮）与引擎侧的 B 类（`engine.ts:668` 等三处，其后**必** `return`）在消费端长得一模一样，无从分辨。后果：`tool_use` 之后直接 `done` —— **工具一次都不执行**，且失败与成功不可分辨（API 错误以 chunk 到达，`stopReason` 恒为 `end_turn`）。**波及**四频道 Bot / schedules / agent API 全部。**先补测试再修**：新增 4 条端到端用例驱动照抄真实顺序的 chunk 序列（A 类 `stop` 夹在中间），红绿实跑 3 红 → 全绿。**端到端实证**（探针 A，真 daemon）：同一提问由「`tool_use` → `done`、0 条 assistant 消息、token 只记第一轮」变为 `Glob → Glob → Read` 三轮全跑通、终答 364 字符。测试 2512 → 2516。**本条为摘要，全文见 [history.md](docs/claude-md-history.md)。**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | 技术委员会 |
...
>
> **完整修订历史**（v1.0.0–v2.48.0，共 100 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
