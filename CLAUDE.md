...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.2
> **最后更新**: 2026-09-14 — 覆盖率实测落地（`pnpm coverage`，行 54.74% / 分支 45.38%）+ 标注 `core/rules-loader.ts` 未接线（生产零调用者、无测试）+ 核实 knip 清单与覆盖率 0% 清单**不相交**（两类信号互补）；v0.81.2 发版
...
- `rules-loader.ts` — 路径作用域规则（.mipham/rules/\*.md → glob 匹配 → 自动注入）；**未接线 —— 生产零调用者且无测试覆盖**：`new RulesLoader` 全历史零出现，`engine.ts:339` 的 `setRulesLoader` 全仓库零调用点（自 e2be832「Sprint 5 — rules system」起就只有定义没有接线），`engine.ts:24` 该 import 的符号仅用于类型位故转换时被丢弃 → 模块从不加载，`engine.ts:369` 守卫永远早退。当前 `.mipham/rules/` 路径规则**不会注入**
...
> **完整修订历史**（v1.0.0–v2.37.2，共 82 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
