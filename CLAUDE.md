...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.94.2
> **最后更新**: 2026-09-23 — **0.85.2：13 笔未发布修复（安全面 4 笔 + 稳定性 / 协议面 9 笔）** —— 安全面四条各自是「**同一形状的两个镜像**」，只修一半就留下同一类洞：① 路径型权限规则按**字面拼写**匹配 ⇒ 一个符号链接既能**擦掉** deny、也能**骗过** allow（`notes.txt` 指向 `.env` 直接放行）；判定改按**落点**（`realpathSync`），两半分两次落地。② 零宽字符剥离集写成**闭区间** `\u{200B}-\u{200F}` ⇒ 连 ZWNJ / ZWJ 一起吞 —— 二者都是**承重**的（ZWNJ 撑波斯 / 阿拉伯词形、ZWJ 让家庭 emoji 保持单字形），写盘静默改写用户内容，而执行层它俩**藏不住任何东西**（`bash -c 'ec<ZWJ>ho hi'` ⇒ `command not found`）。③ MCP OAuth 凭证只按**服务名**复用，而服务名读自项目级 `.mcp.json` ⇒ 同名换端点就把旧 token 交给新主机；凭证加绑定值，任一变化即重授权。④ hook 的三种「没跑完」（超时 / 命令不存在 / 被信号杀死）全塌成同一条空理由，且写 stdin 与子进程退出**赛跑**输掉时的 `EPIPE` 排在退出码之前被读 ⇒ 我们自己的写失败**顶替**了 hook 真正说的话。稳定性 / 协议面 9 笔：半截写赔上全部状态（25 个模块收口 + 读侧形状闸 + 两条两向守卫）；FIFO 配置路径启动**挂死**（`existsSync` 对 FIFO 也为真、`readFileSync` 同步阻塞到有写者 ⇒ 闸门改按**文件类型**判）；对话框里按 Ctrl+C 杀掉整个 CLI（Ink `exitOnCtrlC` 默认 `true`，**任何 handler 之前**就退进程）；后台 agent 的收件箱被广告三处却**无人排空**；子代理没有断环器（连拒 3 次已拒的调用会一轮轮重试到把整个运行耗光）；同一段对话发给两个 provider 是**两个 payload**（上一轮 provider 的报错被存成 `system` 条目、`openai-compat` 原样发出 ⇒ 把 provider 文本提到最高权限角色；汇总指令走同一条路 ⇒ 只有一半 provider 读得到）；`Task output` / `stop` 只认本地 id 空间 ⇒ Agent 工具广告出去的 `bg-…` 永远「找不到」。测试 3,083 → **3,188**（265 文件，0 失败）。
> **前一条（2.94.1）**: 2026-09-23 — **终端 Ctrl-C 曾能把安装打断到一半 —— 0.85.0 的回滚，跑在「它自己也会被杀死」的那个进程里** —— 0.85.0 的「装前快照 / 装后自证 / 失败回滚」修的是 SIGTERM（计时器那条路），而 **Ctrl-C 是另一个信号、另一条路**：终端按 Ctrl-C 时内核把 SIGINT 发给**整个前台进程组** —— CLI 与 npm **一起**死 ⇒ `performUpdate` 的 catch 永不执行，用户手里仍是半截树，而且这次**没有人回滚**。所以「有回滚」≠「Ctrl-C 安全」；判据要问两句：**回滚跑在哪个进程里**、**杀死它的那条信号会不会连它一起带走**。两道守卫，缺一被保下来的都只是一半：① 调用点加 `detached: true` ⇒ npm 自成**进程组**，终端的 SIGINT 到不了它（实测 `execSync` 认可 detached：子进程 pgid ≠ 父进程；**仍阻塞等它退出** 1.01s vs 1.02s ⇒ 自证仍在装完之后；`stdio:'inherit'` 下输出照样打在用户终端上）；② `blockSigintDuringInstall()` ⇒ CLI 自己挂 SIGINT 守位（挂在 `finally` 上 —— 漏撤的话 TUI 里 Ctrl-C **从此永远**没反应，比原来更糟），好让 catch/自证/回滚有机会跑。**4 条新测试**（`update-safety`，真发信号、真阻塞在 `execSync` 里）；**三条负控实跑**：① 删调用点 `detached:true` ⇒ 只红那条断言；② 删 `finally` ⇒ 只红两条「撤掉守位」；③ 守位换空壳 ⇒ **vitest worker 当场被 SIGINT 杀死**（`Errors 1 error`）—— 这条负控的形式是「进程死」而非「断言红」，跑它之前先确认过 vitest 真把它算失败；三处还原后 sha256 逐字相符。**边界**：安装期间 Ctrl-C 会被**忽略**（要中断只能另开终端杀进程 —— 这是本修法认下的代价，不是附带好处）；TUI `/upgrade` 自带 SIGINT 处理，装完后仍可能被投递；根因（npm 全局安装**没有原子换手**）未动 —— 能扛 SIGKILL 的原子换手仍是 **ROADMAP D12**。测试 3,079 → **3,083**（258 文件，0 失败）。
...
测试：3,224 测试（3,222 passed + 2 skipped，0 失败）
...
| --------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| core            | 80      | 1312     | engine / context / permission / hooks / crsi / memory / instructions / paths 等                                                                                             |
...
| ui              | 20      | 239      | commands / input / config-wizard / loop / skill-doctor / ctrl-c                                                                                                             |
| agent           | 11      | 122      | sub-agent / background-registry / pattern-analyzer / effectiveness-tracker                                                                                                  |
...
| shared          | 10      | 85       | arg-validation / deleted-cwd / sanitize / graft / update-async                                                                                                              |
...
| integrity       | 12      | 77       | 引用完整性守卫 + ESLint 规则生效证明 + **遥测契约**（CLI ↔ `apps/telemetry` 逐字段，含 endpoint ↔ vhost 目的地）+ **变异测试范围**（`mutate` 清单 vs 磁盘枚举，延后表明写） |
| telemetry       | 9       | 120      | redact / consent / queue / payload / crash / transport / endpoint / 门面 / 双路径计数一致性                                                                                 |
| **合计**        | **268** | **3224** | **0 失败** ✅（3222 passed + 2 skipped）                                                                                                                                    |
