...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.8
> **最后更新**: 2026-09-15 — `/init` 与 `/setup 1` 预置 provider 恒得 1 家（把运行时 `status === 'active'` 过滤当**代码生成**来源）→ 改接随包清单 `CLOUD_PROVIDERS`（10 家）；顺带修出「带连字符 id 生成非法环境变量名」；测试 2378 → 2388（213 文件，+1 文件）
...
>
> **完整修订历史**（v1.0.0–v2.37.8，共 88 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
