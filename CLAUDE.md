...
测试：2,360 测试（2337 passed + 2 skipped；本机 21 项见「测试」一节注）
...
| **合计**        | **212** | **2360** | **2337 passed + 2 skipped** ✅                                                      |

> **本机 21 项环境闸（非代码问题）**：`core/crsi-*` 与 `core/instructions` 中 21 个测试会 shell 调用真实
> `git`，在 Xcode 许可证未接受的 macOS 上被挡在门外（`/usr/bin/git` 拒绝执行）。已实测确认：把工作区
> **完全回退到干净树**后同样 21 红，与本批改动无关；CI（ubuntu）无此闸，不受影响。
> 本机解法（二选一，均需自行在终端执行）：`sudo xcodebuild -license accept` 一次解决且不影响打包公证；
> 或临时给 `git` 加别名 `alias git=/Library/Developer/CommandLineTools/usr/bin/git`（CLT 版不受此闸限制，
> 但只在交互式 shell 生效）。
