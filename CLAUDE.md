...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.59.0
> **最后更新**: 2026-09-19 — **定向安全审计批次 1（daemon 三处 + Git 执行面，两笔 `f9a6a49`/`851fa1a`）**：四条同形 —— **检查没跑到该跑的地方，或拿着错的对象在比对**。① `daemon/auth.ts` 的 `verifyToken` 调 `Bun.password.constantTimeCompare`，**真实 Bun 无此方法**（1.3.14 实测只有 `hash/hashSync/verify/verifySync`）⇒ 抛 TypeError，**每个远程已认证请求都收 500**；套件却一直绿，因为 `vitest.setup.ts` 的替身**把它补上了** —— **替身比被替身者更有能力，缺陷被藏住而不是被抓住**（只把替身改忠实即当场 **5 红**，含两条 `authMiddleware` ⇒ 藏的是中间件整层）。② `Bun.serve` 未显式给 `development` ⇒ 生产 daemon 跑在**开发模式**，实测**未关 = 67,154 字节 HTML / 关 = 21 字节纯文本**（是否回吐源码帧**未证**，不主张）。③ 轮换写盘成功而 `token` 是 **const** ⇒ 运行中的 server **仍收旧令牌**。④ `Git` 无审批却放行「选项即程序」（`--upload-pack` 等），判据改挂 **argv** 并加 120s 超时。测试 2702 → 2715（233 → 234 文件）。
> **前一条（2.58.0）**: 2026-09-19 — **加固计划 Phase 5（缺口 #24–#26，三条体验项）**：三条同源 —— **工具报给模型的「事实」不完整、或指向错误的根因**。#24 Glob 撞 500 上限直接 `break`，一个字都不说（同一份代码里 Grep 特意加了 `(truncated)`，理由还写在注释里）⇒ 模型把「只看到 500 条」当成「一共 500 条」；改为**先探到第 501 个**再置截断标记（恰好 500 个是完整结果，不谎报），注解在掩码**之后**拼接（`maskGlobOutput` 逐行当路径比对，注解不是路径）。#25 `catch {}` 把 rg 的任何异常都说成「没装 ripgrep」⇒ 真的失败（EAGAIN/EPERM）先静默回退到 `find`，再由回退那侧的 catch 报出「Install ripgrep」，排查方向整条带偏；改为只有 `ENOENT` 才回退，其余原样报 `code: message`，回退自身起不来时同样报真实原因。#26 Edit 审批预览 `slice(0,60)` 按 **UTF-16 码元**切 ⇒ 落点夹在代理对中间就留下半个 emoji（渲染成 U+FFFD）；抽出 `truncateForDisplay` 退到码点边界，同函数另两处预览（Agent 描述 / 未知工具 JSON）用同一规则。测试 2693 → 2702（232 → 233 文件），`/crsi eval` 100/38 不退化。
...
测试：2,715 测试（2713 passed + 2 skipped，0 失败）
...
| tools           | 21      | 362      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
...
| **合计**        | **234** | **2715** | **0 失败** ✅（2713 passed + 2 skipped）                                                                                                                                    |
