...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.57.0
> **最后更新**: 2026-09-19 — **加固计划 Phase 4（缺口 #17–#23，五笔 `b89765f`→`e2aed83`）**：七条一个共同形状 —— **定义在那儿，施加点没接上**。#17 `BLOCKED_PATHS` 是字面量而检查拿 `realpathSync` 后的路径去比 ⇒ macOS 上 `/etc` 是 `/private/etc` 的符号链接，这条检查**在本机是死的**（注释自称「resolved symlinks 之后再查」，说的正是它本该成立的前提），改存规范形。#18 `mergeConfig` 是浅合并 ⇒ 项目配置写 `features.mcp` **整片打掉**用户配的 `features.context`（`crsi`/`skills`/`permissionRules` 同形），改递归，**数组与标量仍替换**。#19 同文件 `loadMcpJson` 手写重建 server 条目 ⇒ 手写清单必然滞后于类型，`request_timeout_ms` 与 `auth` 被静默丢掉。#20 Stop 的拒绝断在中间：`decision`/exit 2/`continue:false` 都在 `runHooks` 归成 `allowed:false`，而引擎的 Stop 分支只读 `decision` ⇒ 在出口统一派生。#21 `loadHookConfigs` 把 matcher 编译成正则并包了壳，`runHooks` 却按**精确相等**过滤 ⇒ 工具事件的 matcher 一直是空的，`Edit`/`Write` 这类 matcher 从不触发；SubagentStart/Stop 连 agent 类型都没往下传。#22 子代理自建四字段 ctx，**根本没有 `permissionSystem`** ⇒ 嵌套工具调用**一次审批都不做**，且 `readFiles` 缺席让「写前先读」守卫的 `add` 成空操作 ⇒ 子代理**永远无法覆盖已存在的文件**。#23 HTTP 传输的 `notificationHandlers` 从未被遍历（stdio 路径正常，故只在 HTTP 上静默）+ `reconnect()` 零调用点、两个事件零消费者 ⇒ 断连后 `/mcp` 一直显示绿的。测试 2661 → 2693（231 → 232 文件），`/crsi eval` 100/38 不退化。
> **前一条（2.56.0）**: 2026-09-18 — **两条真缺口落地（会话恢复健壮性 / 三个状态文件原子写）**：`SessionLog.open()` 是磁盘→内存**唯一**入口却只 `JSON.parse` 不校验，**合法 JSON ≠ 合法事件** ⇒ 一个字节毁掉整份会话；`SessionStore.list()` 的 `try` 包住整个 `for` ⇒ 一个读不出来的文件让 `/resume` 空列表。另三处状态文件裸写**原地截断**改原子写 + 0o600（判据取 **inode** 不取权限）。测试 2645 → 2661（230 → 231 文件）
...
测试：2,693 测试（2691 passed + 2 skipped，0 失败）
...
| --------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| core            | 72      | 1077     | engine / context / permission / hooks / crsi / memory / instructions / paths 等                                                                                             |
| tools           | 21      | 351      | bash / file / exec / skill / agent / scheduling / seam                                                                                                                      |
| daemon          | 33      | 202      | feishu / telegram / 钉钉 / 企业微信渠道 + session / auth / workspace-guard / logger + **引擎接线行为**（`engine-capabilities`）                                             |
| ui              | 11      | 160      | commands / input / config-wizard / loop / skill-doctor                                                                                                                      |
...
| config          | 7       | 42       | credential-crypto / loader-encryption / defaults / settings-json / preferences                                                                                              |
...
| **合计**        | **232** | **2693** | **0 失败** ✅（2691 passed + 2 skipped）                                                                                                                                    |
...
| 版本   | 日期       | 变更内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | 维护人     |
| ------ | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| 2.57.0 | 2026-09-19 | **加固计划 Phase 4（缺口 #17–#23，五笔 `b89765f`→`e2aed83`）** —— 七条一个共同形状：**定义在那儿，施加点没接上**。`security/path.ts` 的 `BLOCKED_PATHS` 存字面量、检查却拿 `realpathSync` 后的路径比 ⇒ macOS `/etc`→`/private/etc`，**这条检查在本机是死的**（注释自称「resolved symlinks 之后再查」，说的正是它本该成立的前提），改存规范形（不存在的条目保留字面量）。`config/loader.ts` 两处：浅合并 ⇒ 写 `features.mcp` **整片打掉**用户配的 `features.context`（`crsi`/`skills`/`permissionRules` 同形），改递归、**数组与标量仍替换**；`loadMcpJson` 手写重建 server 条目 —— 手写清单必然滞后于类型 ⇒ 静默丢掉 `request_timeout_ms` 与 `auth`。`core/hooks.ts` 三处：Stop 的拒绝（`decision`/exit 2/`continue:false`）在 `runHooks` 归成 `allowed:false`，而引擎的 Stop 分支只读 `decision` ⇒ 出口统一派生（PreToolUse 不沾此语义，有用例钉住）；正则 matcher 装了壳、`runHooks` 却按**精确相等**过滤 ⇒ 工具事件的 matcher 一直是空的（**副作用要说清**：`Plan` 现在也会匹配 `EnterPlanMode`/`ExitPlanMode`，`Agent` 也匹配 `ListAgents`）；SubagentStart/Stop 连 agent 类型都没往下传。`agent/sub-agent.ts` 自建四字段 ctx、**根本没有 `permissionSystem`** ⇒ 嵌套工具调用**一次审批都不做**（注入的是钳过的 `subPermission`，**不是**父级的那个，单独用例钉住），`readFiles` 缺席让「写前先读」守卫的 `add` 成空操作 ⇒ 子代理**永远无法覆盖已存在文件**。`mcp/` 的 HTTP 传输 `notificationHandlers` 从未被遍历（stdio 路径正常，故只在 HTTP 上静默）+ `reconnect()` 零调用点、两个事件零消费者 ⇒ 断连后 `/mcp` 一直显示绿；选择**接线而不是删除**（删掉只会更彻底地绿），入口 `/mcp reconnect <name>`，且**刻意不自动重放**失败的工具调用（非幂等副作用不靠重试兜底）；成功文案收窄为 rediscovered —— 注册进中央注册表是**调用方**责任，`connect()` 只填 `connection.tools`。测试 2661 → 2693（231 → 232 文件），`/crsi eval` 100/38 不退化。**本条为摘要，全文见 [history.md](docs/claude-md-history.md)。** | 技术委员会 |
| 2.56.0 | 2026-09-18 | **两条真缺口落地（a 会话恢复健壮性 `da46c0a` / b 三个状态文件原子写 `f5c304c`）** —— (a) ①`SessionLog.open()` 是磁盘→内存的**唯一**入口，却只 `JSON.parse` 不校验 ⇒ 合法 JSON 但不合法事件（如 `compaction/rewrite` 少 `messages`）一路进内存，投影时 `out` 变 `undefined` 就抛，`/resume` 历史投影整份崩 ⇒ 加 `isValidEvent` 逐条闸门，只校验**投影真正有分支**的变体（`session/start`/`assistant/chunk`/`checker/decision` 不校验 —— 校验它们只是仪式），未知 `type` 放行；②`SessionStore.list()` 的 `try` 包住整个 `for` ⇒ 一个读不出来的文件让 `/resume` 一个会话都不显示，改逐文件兜底。两处**覆盖不相交**：撤校验器 ⇒ 3+2 红；撤逐文件 try ⇒ **只 1 红**（权限用例）。(b) 三个写者（`preferences.ts` / `keys-manager.saveKeys` / `tools/system/config.ts`）裸 `writeFileSync` **原地截断**，崩在写中途留不可解析文件，而三个读侧都把「读不出来」吞成**默认值/空** ⇒ 丢的是**整份**；`saveKeys` 还「转了一半」（先写固定名 `.tmp`、不 rename 直接再写目标 ⇒ `.tmp` 是废物、目标仍非原子）。**「原子」的判据取 inode 不取权限**：`{mode:0o600}` 的裸写同样满足 mode 与「不留 .tmp」⇒ 先本机实测判别式（原地重写 ino 不变 / rename 变）再把 **inode 变化**写进四条用例；负控 preferences 退回裸写但留 0o600 ⇒ **只有 inode 那条红**，正是自证。**一次自我纠错（如实记）**：我最初断言 `not.toContain('test-list-bad')` —— 那是**我猜的行为**，加了校验器后该文件**幸存的那条合法事件仍然算数**（救回半份），它会合法地出现在列表里；改成 `messageCount === 1` 并把「全是坏行才整份跳过」拆成独立用例。测试 2645 → 2661（230 → 231 文件）。**本条为摘要，全文见 [history.md](docs/claude-md-history.md)。**                                                                                                                                                                                                                                                                                                         | 技术委员会 |
