...
测试：3,025 测试（3,023 passed + 2 skipped，0 失败）
...
- `permission.ts` — 权限控制（**内部枚举 5 档**：default/acceptEdits/plan/auto/bypassPermissions；Shift+Tab 转盘只有 4 档，`bypassPermissions` **合法但不在转盘上** —— CC 也是这个结构，其描述表列它、转盘数组不列。**第 6 档 `dontAsk` 刻意不加**（我们与 CC 的 `default` 行为本就等于 CC 的 `dontAsk` —— 没有弹窗，`ask` 即硬拒）。`permissionRestrictions`（forbiddenModes/maxAllowedMode）org 级强制降级，请求被禁模式时 fail-closed）
...
| --------------- | ------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| core            | 75      | 1247     | engine / context / permission / hooks / crsi / memory / instructions / paths 等                                                                                             |
...
| **合计**        | **252** | **3025** | **0 失败** ✅（3023 passed + 2 skipped）                                                                                                                                    |
