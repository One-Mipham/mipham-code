...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.47.0
> **最后更新**: 2026-09-15 — **daemon 无头路径修复**：`session-worker` 在 `stop` 上 `break`，而该类型在消费端**被重载**（provider 的 A 类每条流无条件发出 / 引擎的 B 类其后必 `return`）⇒ 引擎的工具循环被整个丢弃。先补 4 条端到端测试（红绿实跑）再删 `break`；探针 A 实测同一路径由「工具 0 执行、0 条 assistant 消息」变为 `Glob → Glob → Read` 三轮跑通。测试 2512 → 2516
...
测试：2,516 测试（2514 passed + 2 skipped，0 失败）
...
| daemon          | 32      | 177      | feishu / telegram / 钉钉 / 企业微信渠道 + session / auth / workspace-guard / logger + **引擎接线行为**（`engine-capabilities`）                                             |
...
| **合计**        | **227** | **2516** | **0 失败** ✅（2514 passed + 2 skipped）                                                                                                                                    |
...
>
> **完整修订历史**（v1.0.0–v2.47.0，共 99 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
