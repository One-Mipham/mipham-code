...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.64.0
> **最后更新**: 2026-09-19 — **hook 的「在哪儿」取的是 `process.cwd()`，不是会话 cwd**（观察②，E8 同族）—— `executeCommand` 用它同时回答两个问题，而两个都不属于本进程：hook **在哪个目录跑**（`spawnSync` 不给 `cwd` 即继承）与它**被告知**自己在哪（stdin 的 `cwd` 字段）。一次性 CLI 恰好都对；daemon 一个进程服务多个会话，`process.cwd()` 不属于任何会话 ⇒ 每个 daemon hook 都落在那里跑、并读到那里。修法是**接线**：`HookContext` 增可选 `cwd`，由 `HookEngine` 在 `runHooks` **一处**盖章。**判据是「两半都断言」**（跑在哪 + 被告知在哪）；三条负控**红集互不相同**（2 / 3 / 1 红）。**未修观察**：hook 掩码仍取用户级而 CLI 的 Bash 拿项目级 —— 属**策略**改动，另案。测试 2809 → 2814（2812 passed + 2 skipped，244 文件不变）。
> **前一条（2.63.0）**: 2026-09-19 — **定向安全审计 N1：项目级 hooks 在问不出信任的地方照样会执行** —— 形状是**「守护点的作用域小于被守护物」**：`<cwd>/.mipham/settings.json` 的 `hooks` 是仓库可控、本进程会 `spawnSync` 起的 shell 命令，而信任闸只在交互式 TTY 上问（无 TTY 直接 return），daemon 这条路径**从不调用它** —— 而 `isCwdAllowed` 只要求会话 cwd 在 daemon 根之内（`isWithin(cwd, daemonRoot)` 或 `isTrusted(cwd)` 二者其一），故未信任的克隆天然满足、可让本进程照其配置起子进程。① 闸从「进程级一次、只在 TTY」下移到**加载点** —— `loadSettingsJson(cwd, {includeProjectHooks})` **默认 false**（读项目 hooks 重新定义成一次信任行为），三处调用点各自表态：`index.tsx` 与 daemon `hooksFor` 按 `isTrusted()` 判定，`/hooks` 显式传 `true`（它只是**显示**，且 TUI 只在信任落定后才启动）；`permissions` 不受该开关影响（那是模式上限的问题，不是信任的问题）。② hook 子进程的 env 一并收口：裸 `spawnSync` 把整个环境交给 hook，改用 Bash 自 E1 起就在用的同一条 `filterEnv` 掩码 —— 只取用户级配置（`HookContext` 不带 cwd，无会话作用域可读项目段）—— ⚠️ **括号内理由自 2.64.0 起已不成立**（`HookContext` 此后带 cwd）；**决定**仍有效，收口属**策略**改动。**判据不许宣布一次没发生过的跳过**：新增 `projectHooksSkipped`，由**同一份解析**回答「到底扣下了没有」，故告警不可能跑在事实前面 —— 第一版直接在调用点写警告，全绿那一跑打出 **8 条**（含无 settings.json 的工作区），改后收敛到**恰好 1 条**，并补用例钉住「没有项目 hooks 就不许告警」的反方向。三条负控互不相同：`readHooks` 恒 `true` ⇒ 3 红、无条件报告跳过 ⇒ 1 红、摘掉 `env` 传参 ⇒ 1 红。测试 2800 → 2809（2807 passed + 2 skipped，244 文件不变）。
...
测试：2,814 测试（2812 passed + 2 skipped，0 失败）
...
| ui              | 13      | 169      | commands / input / config-wizard / loop / skill-doctor                                                                                                                      |
...
| config          | 9       | 64       | credential-crypto / loader-encryption / defaults / settings-json / preferences                                                                                              |
| plugin          | 2       | 32       | claude-plugin / plugin-manager                                                                                                                                              |
| artifacts       | 1       | 6        | manifest                                                                                                                                                                    |
| agent-view      | 1       | 9        | agent-view-manager                                                                                                                                                          |
| e2e             | 1       | 8        | full-pipeline                                                                                                                                                               |
| integrity       | 8       | 50       | 引用完整性守卫 + ESLint 规则生效证明 + **遥测契约**（CLI ↔ `apps/telemetry` 逐字段，含 endpoint ↔ vhost 目的地）+ **变异测试范围**（`mutate` 清单 vs 磁盘枚举，延后表明写） |
| telemetry       | 9       | 120      | redact / consent / queue / payload / crash / transport / endpoint / 门面 / 双路径计数一致性                                                                                 |
| **合计**        | **244** | **2814** | **0 失败** ✅（2812 passed + 2 skipped）                                                                                                                                    |
