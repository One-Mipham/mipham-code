...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.37.5
> **最后更新**: 2026-09-15 — 三处修复：`READER_COMMANDS` 补 fmt/column 等 20 条读者命令（`Read(secret)` 曾被 `fmt secret` 绕过）、MCP `list_changed` 合并刷新（250ms 去抖 + 2000ms 上限）、`/clear`//`/resume` 重置 `readFiles`；测试 2293 → 2304（core 969→976 / mcp 79→83，+11）
...
测试：2,304 测试（2302 passed + 2 skipped）
...
| --------------- | ------- | -------- | ----------------------------------------------------------------------------------- |
| core            | 71      | 976      | engine / context / permission / hooks / crsi / memory / instructions 等             |
...
| **合计**        | **208** | **2304** | **0 失败** ✅（2302 passed + 2 skipped）                                            |
...
> **完整修订历史**（v1.0.0–v2.37.5，共 85 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
