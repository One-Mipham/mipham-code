...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.42.0
> **最后更新**: 2026-09-15 — **T1b 接收端上线 + 客户端 `schemaVersion: 2` 撤帧**（详见「遥测与崩溃上报」段与修订历史 2.42.0）：端点 `log.onemipham.com` 在主机 2 起服务（公开只写、只存按接收日分区的维度聚合），实机验收 = `--check` no drift + 服务 active + 真 CLI 端到端两跑；`stackFrames` 不再上网（服务端从来不存），帧只留本进程内存；**已知偏离：该端点实际 TLS 1.2+1.3，非 1.3-only**（主机 2 上握手版本由该地址的默认 server 决定 —— 版本在 ClientHello 时定死、早于 SNI 回调，官方 wontfix 到 1.29.2），属集团 §二 例外申请范畴。测试 2523 → 2525（224 文件）
...
>
> **完整修订历史**（v1.0.0–v2.42.0，共 94 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)。
