...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.41.0
> **最后更新**: 2026-09-15 — T1b 前置：`command_name` 基数无界修复 —— `parseSlashCommand` 返回用户敲的原文、`app.tsx` 又在注册表查询**之前**计数，于是任意输入都能新造一条 `command_calls.*` 序列，而 `MAX_LABEL_LENGTH` 只截值的长度、不限键的个数；新增 `commandLabelFor()` 收敛到 `/unknown` + `getCommandLabelNames()` 作为 allowlist 派生源（138 → 140 条），并加守卫扫 `app.tsx` 的命令字面量防两处清单漂移。测试 2503 → 2506（223 文件不变）
...
| **合计**        | **224** | **2521** | **0 失败** ✅（2519 passed + 2 skipped）                                                                                                                          |
...
> 2026-09-15 已在本机执行，全量 **2519 passed + 2 skipped / 0 失败**。
