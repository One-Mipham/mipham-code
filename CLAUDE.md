...
> **产品**: 多模型开源智能编程终端
> **版本**: 2.55.0
> **最后更新**: 2026-09-18 — **#12 后半：删掉并行版本机制 + 三条就地修**：两个版本机制只有 manifest 归档活在链路上，另一个（`versioning.ts`）的唯一写者 `saveVersion` **零调用点** —— 「恢复上一版」从未被喂过、它维护的 `current.html` 从未存在过 ⇒ 删除，`/artifact` 按名字订阅改读工具**真正写下的那份文件**、找不到即 404。删除当场暴露一个真缺陷：`stop()` 只调 `server.close()`（只停止**接收新连接**，已建立的 socket 继续被服务）⇒「已停止」的服务器仍在旧端口应答，而它的目录已被测试删掉 —— 新 SSE 用例单跑绿、全量约 3/8 红。另三条执行中撞见：manifest 去重只按 `name`（全局一份索引，两会话同名互相顶掉）、读失败静默当空而写路径整份重写（坏索引被覆盖成「只有这一条」）⇒ 读写分两条路径 + 隔离坏索引 + 原子写；无 `cwd` 的旧 cron 任务仍照发（刻意）⇒ 列表标 `[无归属]` 并给出处置办法。测试 2658 → 2645（231 → 230 文件）
> **前一条（Phase 3）**: 2026-09-18 — **数据落点与并发写（Phase 3）**：五条同形 —— 同一个事实被两处各自算一遍。artifact 的目录（工具写 `<cwd>/artifacts`、服务端根在 `<cwd>/.mipham/artifacts` ⇒ 工具印的 URL **必然 404**）收进 `artifactsRoot()`；`archiveVersion` 源文件不在时照样推进版本号（manifest 里留一版磁盘上没有的）；`atomicWriteFileSync` 的固定 `.tmp` 名在共享路径上会**丢写**；cron 的 prompt 塞进**当前会话**执行 ⇒ 加 `cwd` 归属并按 cwd 过滤；Read 声称支持 offset/limit 却先整读再切片（超 50 MB 一律报错，而错误建议的「use offset/limit」**不可执行**）。四条负对照各自只咬一条，其中两条最初没咬住是测试自己的问题（边界不可达 / 凭据规则拼的是原始 tmpdir 而函数返回 realpath）。测试 2639 → 2658（228 → 231 文件）
...
| 2.53.0 | 2026-09-18 | **搜索工具的截断与错误分类（Phase 2，清单 #8/#9/#10，`5202290`）** —— 三个缺陷都长在同一个函数的两条路径上：①**`rg` 快路径完全不走输出上限** —— `truncateGrepOutput` 只有 find 回退一个调用点，rg 的 exit 0 与 exit 2 两个分支都是原样返回（46 万字符实测超限）；②**find 回退把「根本没搜成」读成「无匹配」** —— BSD find 在「目录不可读 / grep 正则非法 / grep 不在 PATH」三种情形下**退出码都是 1**，与「真没搜到」同形，三种情形的端到端输出逐字同为 `(no matches)`；③**stderr 全程无人读** —— `runSearch` 以 `stderr:'pipe'` 起进程却从不消费，先读满 stdout 再轮 stderr ⇒ 子进程写满 ~64 KB 管道缓冲即阻塞、永不退出、stdout 也永远读不到 EOF，只能等 120 s 超时兜底（**实测**：300 KB stderr 的进程在原实现下 5 s 未退出，`timedOut` 为真）。**三处修法**：rg 两个分支都套上限；find 的退出码 1 改用 **stderr 是否为空**分辨（只有静默退 1 才是「无匹配」）；`runSearch` 并发读两条管道并把 stderr 一并回传。**一处细节**：exit 2 的「部分结果 + 注解」分支改成**先截断正文、再拼注解** —— 注解拼在截断之内会被自己切掉，模型拿到的是一句没头没尾的提示（负控 5 单独把这条性质钉出来）。**四条负控，各自只咬一条**：摘掉 rg 截断 ⇒ 1 红；退出码 1 回到「一律无匹配」⇒ 1 红（**正对照**：stderr 为空时仍须报「无匹配」，否则就是把同一个 bug 镜像过来）；两条管道改回顺序读 ⇒ 1 红（恰好是那条 5 s 超时的用例）。**顺带修一处测试形状**：旧用例给 stdout 与 stderr 传**同一个**流对象 —— bun 从不这样返回，而并发读会把同一个流读两次（流被锁）；换成两个独立流并抽出 `fakeProc` 共用。测试 2633 → 2639（228 文件不变），tools 328 → 334。**本条为摘要，全文见 [history.md](docs/claude-md-history.md)。**                                                                                  |

> **本表只留最近 3 行**（滚动窗口，同上方约定）—— 上表是**摘要**，每条全文与被挤掉的行都在 history.md。
> **完整修订历史**（v1.0.0–v2.55.0，共 107 条）→ [`docs/claude-md-history.md`](docs/claude-md-history.md)：
